...
data type. As discussed in Section `[objecttypes] <#objecttypes>`__, the “wire" is full-duplex meaning that it can send
data in both directions, and it only provides the latest value. When the ``OutValue`` of one ``WireConnection`` is
