...
public:
	...
	void Connect(const ParseConnectionURLResult& url, boost::string_ref noden, uint32_t endpoint, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		...
		RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&BluetoothConnector::DoConnect, this->shared_from_this(), boost::protect(handler)), true);
	}
	...
	void DoConnect(boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		...
		if (devices.empty())
		{
			handler(RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Bluetooth node not found"));
			return;
		}
		...
		{
			boost::thread(boost::bind(&BluetoothConnector::DoConnect1, this->shared_from_this(), a1, boost::protect(handler), key));
			active_keys.push_back(key);
			key++;
		}
	}

	void DoConnect1(btaddr addr, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler, int32_t key)
	{
		{
			boost::mutex::scoped_lock lock(this_lock);
		...
		{
			handler(RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Could not initialize Bluetooth connection"));
		}
	}

	void DoConnect_err(RR_SHARED_PTR<RobotRaconteurException> exp, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler, int32_t key)
	{
		boost::mutex::scoped_lock lock(this_lock);
		if (!connecting) return;
	...
	bool ReadSdpRecord(boost::asio::mutable_buffer& raw_record, device_info& info)
	{
		try
		{
			...
			if (record_h.get<1>() != boost::asio::buffer_size(raw_record)) return false;
			...
			while (boost::asio::buffer_size(raw_record) > 0)
			{
				...
				if (data_h.get<1>() == 0) break;

				if ((data_id == 0xF001 || data_id == 0xF002) && data_h.get<0>() == 4)
				{
					std::string s;
					if (data_h.get<1>() > 512) continue;
					s.resize(data_h.get<1>());
					if (boost::asio::buffer_copy(boost::asio::buffer(&s[0], data_h.get<1>()), raw_record) != data_h.get<1>())
					{
						break;
					}
