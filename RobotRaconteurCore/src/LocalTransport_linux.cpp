...
{
	return inotify_add_watch(notify_fd, path.c_str(), IN_ATTRIB | IN_MODIFY | IN_DELETE | IN_DELETE_SELF | IN_ONLYDIR | IN_CREATE);
}
...
void LinuxLocalTransportDiscovery::Init()
{
	...
	if (!shutdown_evt.get() < 0) throw InternalErrorException("Internal error");

	public_evt=RR_MAKE_SHARED<LocalTransportUtil::FD>(inotify_init1(IN_NONBLOCK | IN_CLOEXEC));
	if (public_evt->fd() < 0) throw InternalErrorException("Internal error");
	...
	while(true)
	{
		...
		if (!private_dir)
		{
			try
			{
				...
				if (private_path)
				{
					RR_SHARED_PTR<LinuxLocalTransportDiscovery_dir> private_dir1=RR_MAKE_SHARED<LinuxLocalTransportDiscovery_dir>();
		...
		for (std::map<std::string,RR_SHARED_PTR<LinuxLocalTransportDiscovery_dir> >::iterator e=public_user_dirs.begin(); e!=public_user_dirs.end(); e++)
		{
		...
		if (public_wd > 0)
		{
			try
			{
				if (public_path)
				{
					...
					{
						for (boost::filesystem::directory_iterator dir_itr(*public_path); dir_itr!=boost::filesystem::directory_iterator(); dir_itr++)
						{
							try
							{
								boost::filesystem::path new_path=*dir_itr;
								std::string username=new_path.filename().string();
								RR_SHARED_PTR<LinuxLocalTransportDiscovery_dir> d=RR_MAKE_SHARED<LinuxLocalTransportDiscovery_dir>();
								if(d->Init(new_path))
								{
			...
			for (char* ptr = buf; ptr < buf + len; ptr += sizeof(struct inotify_event) + event->len)
			{
				...
				if (event->mask & IN_CREATE)
				{
					...
					if(boost::filesystem::is_directory(new_path))
					{
						...
						if(d->Init(new_path))
						{
							std::map<std::string,RR_SHARED_PTR<LinuxLocalTransportDiscovery_dir> >::iterator e_find=public_user_dirs.find(username);
