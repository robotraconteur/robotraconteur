...
namespace detail
{
	...
	static void DarwinLocalTransportDiscovery_FSCallback(ConstFSEventStreamRef stream, void *callbackInfo, size_t numEvents, void *evPaths, const FSEventStreamEventFlags evFlags[], const FSEventStreamEventId evIds[])
	{
		DarwinLocalTransportDiscovery* this_=(DarwinLocalTransportDiscovery*)callbackInfo;
		this_->Refresh();
	}
	...
	void DarwinLocalTransportDiscovery::run()
	{
		...
		try {
			...
			if (!cf_paths[0] || !cf_paths[1]) throw SystemResourceException("");
			CFArrayRef paths = CFArrayCreate(NULL, (const void**)cf_paths, 2, NULL);
			if (!paths) throw SystemResourceException("");

			CFAbsoluteTime latency = 3.0;
			FSEventStreamContext streamContext={0, this, NULL, NULL, NULL};
			
			stream = FSEventStreamCreate(NULL, &DarwinLocalTransportDiscovery_FSCallback, &streamContext, paths, kFSEventStreamEventIdSinceNow, latency, kFSEventStreamCreateFlagNone);
			if (!stream) throw SystemResourceException("");
			...
			timer = CFRunLoopTimerCreate(kCFAllocatorDefault, FireTime, timer_interval, 0, 0, DarwinLocalTransportDiscovery_TimerCallback, &TimerContext);
			if (!timer) throw SystemResourceException("");
