...
void AndroidHardwareDirector::ConnectBluetooth(RR_SHARED_PTR<HardwareTransport> parent, const ParseConnectionURLResult& url, const std::string& noden, uint32_t endpoint, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
{
	...
	if (!director)
	{
		RobotRaconteurNode::TryPostToThreadPool(parent->GetNode(), boost::bind(handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Internal bluetooth error")),true);
		return;
	}
...
{
	if (!p) throw InvalidOperationException("");
	...
	RR_SHARED_PTR<boost::asio::generic::stream_protocol::socket> sock = RR_MAKE_SHARED<boost::asio::generic::stream_protocol::socket>(boost::ref(p->parent->GetNode()->GetThreadPool()->get_io_service()), protocol, fds[0]);
