...
{
	if (lib_handle) throw InvalidOperationException("libusb functions already loaded");
...
LibUsb_Transfer::LibUsb_Transfer(RR_SHARED_PTR<LibUsb_Functions> f, RR_SHARED_PTR<libusb_device_handle> device_handle, RR_SHARED_PTR<LibUsbDeviceManager> device_manager)
{
	this->f=f;
...
LibUsb_Transfer_control::LibUsb_Transfer_control(RR_SHARED_PTR<LibUsb_Functions> f, RR_SHARED_PTR<libusb_device_handle> device_handle, RR_SHARED_PTR<LibUsbDeviceManager> device_manager)
	: LibUsb_Transfer(f, device_handle, device_manager)
{
	temp_buf=NULL;
}

void LibUsb_Transfer_control::FillTransfer(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	size_t s=8 + boost::asio::buffer_size(buf);
	transfer=f->libusb_alloc_transfer(0);
	if (!transfer) throw SystemResourceException("Memory error");
	...
	libusb_fill_control_transfer(transfer, device_handle.get(), temp_buf, &LibUsbDeviceManager::transfer_complete, this, 5000);
...
void LibUsb_Transfer_bulk::FillTransfer(uint8_t ep, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	transfer=f->libusb_alloc_transfer(0);
	if (!transfer) throw SystemResourceException("Memory error");
	this->data_buf=buf;

	libusb_fill_bulk_transfer(transfer, device_handle.get(), ep, boost::asio::buffer_cast<uint8_t*>(buf), boost::asio::buffer_size(buf), &LibUsbDeviceManager::transfer_complete, this, 0);
...
LibUsb_Transfer_bulk::LibUsb_Transfer_bulk(RR_SHARED_PTR<LibUsb_Functions> f, RR_SHARED_PTR<libusb_device_handle> device_handle, RR_SHARED_PTR<LibUsbDeviceManager> device_manager)
	: LibUsb_Transfer(f, device_handle, device_manager)
{
...
LibUsbDeviceManager::LibUsbDeviceManager(RR_SHARED_PTR<HardwareTransport> parent)
...
static void LibUsb_Functions_libusb_exit(RR_SHARED_PTR<LibUsb_Functions> f, libusb_context* c)
{
	f->libusb_exit(c);
}
...
static UsbDeviceStatus LibUsbDevice_open_device(RR_SHARED_PTR<LibUsbDeviceManager> m, RR_SHARED_PTR<LibUsb_Functions> f, RR_SHARED_PTR<libusb_device> device, RR_SHARED_PTR<void>& device_handle)
{
...
bool LibUsbDeviceManager::InitUpdateDevices()
{
	...
	if (!context)
	{
		...
		running=true;
		usb_thread=boost::thread(boost::bind(&LibUsbDeviceManager::UsbThread, RR_STATIC_POINTER_CAST<LibUsbDeviceManager>(shared_from_this())));
	}
...
std::list<UsbDeviceManager_detected_device> LibUsbDeviceManager::GetDetectedDevicesPaths()
{
	...
	for (ssize_t i=0; i < device_count; i++)
	{
		...
		if (bus_num <= 0 || port_num <= 0) continue;

		std::wstring ws_path=boost::lexical_cast<std::wstring>(bus_num) + std::wstring(L",") + boost::lexical_cast<std::wstring>(port_num);
		...
		for (uint8_t j=0; j<config_desc->bNumInterfaces; j++)
		{
			...
			{
				const robotraconteur_usb_common_descriptor* c1 = boost::asio::buffer_cast<const robotraconteur_usb_common_descriptor*>(extra_desc);
			...
			{
				{
					...
					try
					{
						std::string message="Internal usb event loop failure: " + boost::lexical_cast<std::string>(rv);
...
void LibUsb_Functions_libusb_close(RR_SHARED_PTR<LibUsb_Functions> f, libusb_device_handle* h)
{
	f->libusb_close(h);
}

void LibUsbDeviceManager::LibUsbCloseDevice(RR_WEAK_PTR<LibUsbDeviceManager> d, RR_SHARED_PTR<LibUsb_Functions> f, libusb_device_handle* h)
{
...
int LibUsbDeviceManager::OnUsbHotplugEvent(libusb_context* ctx, libusb_device* device, libusb_hotplug_event event, void* user_data)
{
	return 0;
}
...
void LibUsbDeviceManager::transfer_complete(libusb_transfer* transfer)
{
...

	{
		...
		if (m)
		{
			...
			{
				std::map<libusb_device_handle*, boost::function<void()> >::iterator e=m->closing_device_handles.find(t1->device_handle.get());
				if (e!=m->closing_device_handles.end())
				{
					int count=0;
					for (LibUsb_Transfer::manager_transfer_list_t::iterator e2=m->manager_transfer_list.begin(); e2!= m->manager_transfer_list.end(); e2++)
					{
...
void LibUsbDeviceManager::DrawDownRequests(RR_SHARED_PTR<libusb_device_handle> h, boost::function<void()> handler)
{
	...
	int count=0;
	for (LibUsb_Transfer::manager_transfer_list_t::iterator e=manager_transfer_list.begin(); e!= manager_transfer_list.end(); e++)
	{
...
LibUsbDevice_Initialize::LibUsbDevice_Initialize(RR_SHARED_PTR<UsbDevice> parent, RR_SHARED_PTR<LibUsb_Functions> f, const UsbDeviceManager_detected_device& detected_device)
...
void LibUsbDevice_Initialize::AsyncControlTransfer(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler, RR_SHARED_PTR<void> dev_h)
{
	boost::mutex::scoped_lock lock(this_lock);
...
//Call with lock
void LibUsbDevice_Initialize::AsyncControlTransferNoLock(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler, RR_SHARED_PTR<void> dev_h)
{
	...
	boost::static_pointer_cast<LibUsb_Transfer_control>(t)->FillTransfer(bmRequestType, bRequest, wValue, wIndex, buf, handler);
...
//Call with lock
UsbDeviceStatus LibUsbDevice_Initialize::ReadPipeSettings(RR_SHARED_PTR<void> dev_h, RR_SHARED_PTR<UsbDevice_Settings>& settings)
{
	...
	for (uint8_t i=0; i<interface_desc->bNumEndpoints; i++)
	{
		...
		if (USB_ENDPOINT_TYPE_BULK(ep->bmAttributes)
				&& USB_ENDPOINT_DIRECTION_IN(ep->bEndpointAddress)
				&& !in_found)
		{
			in_found=true;
		...
		if (USB_ENDPOINT_TYPE_BULK(ep->bmAttributes)
				&& USB_ENDPOINT_DIRECTION_OUT(ep->bEndpointAddress)
				&& !out_found)
		{
			out_found=true;
...
UsbDeviceStatus LibUsbDevice_Initialize::ReadInterfaceSettings(RR_SHARED_PTR<void> dev_h, RR_SHARED_PTR<UsbDevice_Settings>& settings)
{
	RR_SHARED_PTR<libusb_device_handle> h=RR_STATIC_POINTER_CAST<libusb_device_handle>(dev_h);
	...
	{
		const robotraconteur_usb_common_descriptor* c1 = boost::asio::buffer_cast<const robotraconteur_usb_common_descriptor*>(desc);
...
//LibUsbDevice_Claim
LibUsbDevice_Claim::LibUsbDevice_Claim(RR_SHARED_PTR<UsbDevice> parent, RR_SHARED_PTR<LibUsb_Functions> f, const UsbDeviceManager_detected_device& detected_device)
...
void LibUsbDevice_Claim::AsyncControlTransfer(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler, RR_SHARED_PTR<void> dev_h)
{
	boost::mutex::scoped_lock lock(this_lock);
	AsyncControlTransferNoLock(bmRequestType, bRequest, wValue, wIndex, buf, handler, dev_h);
}

void LibUsbDevice_Claim::AsyncControlTransferNoLock(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler, RR_SHARED_PTR<void> dev_h)
{
	RR_SHARED_PTR<LibUsbDeviceManager> m1=m.lock();
	if (!m1)
	{
...
void LibUsbDevice_Claim::AsyncReadPipe(uint8_t ep, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	boost::mutex::scoped_lock lock(this_lock);
	AsyncPipeOp(ep, buf, handler);
}

void LibUsbDevice_Claim::AsyncReadPipeNoLock(uint8_t ep, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	AsyncPipeOp(ep, buf, handler);
}

void LibUsbDevice_Claim::AsyncWritePipe(uint8_t ep, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	boost::mutex::scoped_lock lock(this_lock);
	AsyncPipeOp(ep, buf, handler);
}

void LibUsbDevice_Claim::AsyncWritePipeNoLock(uint8_t ep, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	AsyncPipeOp(ep, buf, handler);
}

void LibUsbDevice_Claim::AsyncPipeOp(uint8_t ep, boost::asio::mutable_buffer& buf, boost::function< void(const boost::system::error_code&, size_t) > handler)
{
	RR_SHARED_PTR<LibUsbDeviceManager> m1=m.lock();
	if (!m1)
	{
...
UsbDeviceStatus LibUsbDevice_Claim::ClaimDevice(RR_SHARED_PTR<void>& dev_h)
{
	RR_SHARED_PTR<LibUsbDeviceManager> m1=m.lock();
	if (!m1) return Error;
	UsbDeviceStatus res = LibUsbDevice_open_device(m1, f, RR_STATIC_POINTER_CAST<libusb_device>(detected_device.handle), dev_h);
	if (res != Open)
	{
		return res;
	}
...
LibUsbDevice::LibUsbDevice(RR_SHARED_PTR<LibUsbDeviceManager> parent, RR_SHARED_PTR<LibUsb_Functions> f, const UsbDeviceManager_detected_device& device)
