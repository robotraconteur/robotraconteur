...
namespace detail
{
	...
	class HardwareTransport_discovery : public RR_ENABLE_SHARED_FROM_THIS<HardwareTransport_discovery<usb_manager, bluetooth_connector> >
	{
	public:
		class refresh_op
		{
		...
		HardwareTransport_discovery(RR_SHARED_PTR<HardwareTransport> parent, const std::vector<std::string>& schemes, RR_SHARED_PTR<usb_manager> usb, RR_SHARED_PTR<bluetooth_connector> bt)
		{
			this->parent = parent;			
			this->usb = usb;
			this->bt = bt;
		...
		RR_SHARED_PTR<HardwareTransport> GetParent()
		{
			...
			if (!p) throw InvalidOperationException("HardwareTransport closed");
			return p;
		}
		...
		void GetUsbDevices(boost::function<void(RR_SHARED_PTR<std::vector<NodeDiscoveryInfo> >)> handler)
		{
			try
			{
				...
				m->UpdateDevices(boost::bind(&HardwareTransport_discovery::GetUsbDevices1, this->shared_from_this(), boost::protect(handler)));
			}
			catch (std::exception&)
			{
		...
		void GetBluetoothDevices(boost::function<void(RR_SHARED_PTR<std::vector<NodeDiscoveryInfo> >)> handler)
		{
			...
			{
				if (RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&HardwareTransport_discovery::GetBluetoothDevices1, this->shared_from_this(), op, a)))
				{
					op->count++;
				}
			}
		...
		void GetAll(boost::function<void(RR_SHARED_PTR<std::vector<NodeDiscoveryInfo> >)> handler, int32_t timeout)
		{
			...
			if (timeout >= 0)
			{
				...
				RobotRaconteurNode::asio_async_wait(node, op->timer, boost::bind(&HardwareTransport_discovery::GetAll2, this->shared_from_this(), boost::asio::placeholders::error, op));
			}
			...
			if (boost::range::find(schemes, "rr+usb") != schemes.end())
			{
				if (RobotRaconteurNode::TryPostToThreadPool(n, boost::bind(&HardwareTransport_discovery::GetBluetoothDevices2, this->shared_from_this(), op)))
				{
					op->count++;
				}
			}
	...
	protected:	
		...
		void GetBluetoothDevices1(RR_SHARED_PTR<refresh_op> op, typename bluetooth_connector::btaddr_type addr)
		{
			...
			{
				if (!e.nodeid_str || !e.nodename_str) continue;
		...
		void OnBluetoothChanged(typename bluetooth_connector::btaddr_type addr)
		{
			...
			typedef typename std::list<boost::tuple<typename bluetooth_connector::btaddr_type, boost::posix_time::ptime> >::iterator e_type;
