...
static std::string TcpTransport_socket_remote_endpoint(RR_SHARED_PTR<boost::asio::ip::tcp::socket>& socket)
{
...
static std::string TcpTransport_socket_local_endpoint(RR_SHARED_PTR<boost::asio::ip::tcp::socket>& socket)
{
	boost::system::error_code ec;
...
namespace detail
{
	...
	void TcpConnector::Connect(std::vector<std::string> url, uint32_t endpoint, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "TcpConnector begin connection with candidate URLs: " << boost::join(url,", "));
		...
		BOOST_FOREACH (std::string& e, url)
		{
			...
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, endpoint, "Candidate URL \"" << e << "\" error: " << exp.what());
				if (url.size() == 1) throw;
			...
			{
				{
					...
					parent->AddCloseListener(connect_timer, boost::bind(&boost::asio::deadline_timer::cancel, RR_BOOST_PLACEHOLDERS(_1)));
				}
	...
	void TcpConnector::connect2(int32_t key, const boost::system::error_code& err, boost::asio::ip::tcp::resolver::results_type results, boost::function<void(RR_SHARED_PTR<TcpTransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > callback)
	{
		...
		try
		{
			...
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Found candidate endpoints: " << TcpConnector_log_candidate_endpoints(candidate_endpoints));
		...
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Error resolving candidate endpoints: " << exp.what());
			...
			{				
				...
				RR_SHARED_PTR<boost::signals2::scoped_connection> sock_closer =
					RR_MAKE_SHARED<boost::signals2::scoped_connection>(
						parent->AddCloseListener(sock, boost::bind(&boost::asio::ip::tcp::socket::close, RR_BOOST_PLACEHOLDERS(_1)))
						);
	...
	void TcpConnector::connect4()
	{
		...
		if (all_stopped)
		{
			...
			BOOST_FOREACH (RR_SHARED_PTR<RobotRaconteurException>& e, errors)
			{
				...
				if (e2)
				{
					ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Error connecting to remote host:" << e2->what());
			...
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Error connecting to remote host:" << errors.back()->what());
	...
	void TcpConnector::connected_callback(RR_SHARED_PTR<boost::asio::ip::tcp::socket> socket, RR_SHARED_PTR<boost::signals2::scoped_connection> socket_closer, int32_t key, const boost::system::error_code& error)
	{
		if (error)
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Candidate endpoint " << TcpTransport_socket_remote_endpoint(socket) << " connection error: " << error.message());
		...
		try
		{
			...
			{				
				...
				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Candidate endpoint " << TcpTransport_socket_remote_endpoint(socket) << " connected!");

				socket->set_option(boost::asio::ip::tcp::socket::linger(true, 5));
				boost::function<void(RR_SHARED_PTR<boost::asio::ip::tcp::socket>, RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> cb = boost::bind(&TcpConnector::connected_callback2, shared_from_this(), RR_BOOST_PLACEHOLDERS(_1), key2, RR_BOOST_PLACEHOLDERS(_2), RR_BOOST_PLACEHOLDERS(_3));
		...
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Candidate endpoint " << TcpTransport_socket_remote_endpoint(socket) << " connection error: " << exp.what());
			handle_error(key, boost::system::error_code(boost::system::errc::io_error, boost::system::generic_category()));
		}
	}	
	...
	void TcpConnector::connected_callback2(RR_SHARED_PTR<boost::asio::ip::tcp::socket> socket, int32_t key, RR_SHARED_PTR<ITransportConnection> connection, RR_SHARED_PTR<RobotRaconteurException> err)
	{
		if (err)
		{
			if (connection)
			{
				try
				{
					ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Closing losing candidate connection: " << TcpTransport_socket_remote_endpoint(socket));
					connection->Close();
				}
				catch (std::exception&) {}
			}
			//callback(RR_SHARED_PTR<TcpTransportConnection>(),err);
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Error attaching transport to socket: " << TcpTransport_socket_remote_endpoint(socket));
			handle_error(key, err);
			return;
		...
		try
		{
			...
			try
			{
				...
				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "TcpConnector connected transport to " << TcpTransport_socket_remote_endpoint(socket));
				ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport connected transport to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
				callback(boost::dynamic_pointer_cast<ITransportConnection>(connection), RR_SHARED_PTR<RobotRaconteurException>());
			}
			catch (std::exception& exp)
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, endpoint, "Error in transport connect handler: " << exp.what());
				RobotRaconteurNode::TryHandleException(node, &exp);
			}
		}
		catch (std::exception& exp2)
		{
			ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, endpoint, "Error finalizing transport connection " << exp2.what());
			handle_error(key, boost::system::error_code(boost::system::errc::io_error, boost::system::generic_category()));
		}
	...
	void TcpConnector::handle_error(const int32_t& key, RR_SHARED_PTR<RobotRaconteurException> err)
	{
		...
		BOOST_FOREACH (RR_SHARED_PTR<RobotRaconteurException> e, errors)
		{
			...
			if (e2)
			{
				ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport failed to connect: " << e2->what());
				callback(RR_SHARED_PTR<TcpTransportConnection>(), e2);
				return;
			}
		}
		...
		if (!s)
		{
			try
			{
				ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport failed to connect: " << err->what());
				callback(RR_SHARED_PTR<TcpTransportConnection>(), err);
			}
			catch (std::exception& exp)
			{
			...
			)
	{
		if (err)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, 0, "TcpTransport accepted socket stream attach failed " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket) << " with error: " << err->what());
			handler(RR_SHARED_PTR<boost::asio::ip::tcp::socket>(), RR_SHARED_PTR<ITransportConnection>(), err);
			return;
		}

		ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, 0, "TcpTransport accepted incoming transport connection " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
		...
		)
	{
		if (ec)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, 0, "TcpTransport accepted socket closed " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket) << " with error: " << ec.message());
		...
		try
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, 0, "TcpTransport accepted socket attaching websocket protocol " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
			RR_SHARED_PTR<TcpTransportConnection> t = RR_MAKE_SHARED<TcpTransportConnection>(parent, url, true, local_endpoint);
			boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(handler, socket, t, RR_BOOST_PLACEHOLDERS(_1));
		...
		boost::function<void(RR_SHARED_PTR<boost::asio::ip::tcp::socket>, RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)>& handler)
	{
		...
		if (seed == "RRAC")
		{
			try
			{
				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, 0, "TcpTransport accepted socket attaching raw protocol " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
				RR_SHARED_PTR<TcpTransportConnection> t = RR_MAKE_SHARED<TcpTransportConnection>(parent, url, true, local_endpoint);
				boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpAcceptor::AcceptSocket6,shared_from_this(),RR_BOOST_PLACEHOLDERS(_1),socket,t,handler);
				t->AsyncAttachSocket(socket, h);
				parent->AddCloseListener(t, &TcpTransportConnection::Close);
			}
			catch (std::exception&)
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, 0, "TcpTransport accepted socket attach stream failed " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
				RobotRaconteurNode::TryPostToThreadPool(parent->GetNode(),boost::bind(handler, RR_SHARED_PTR<boost::asio::ip::tcp::socket>(), RR_SHARED_PTR<TcpTransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Could not connect to service")),true);
			}
		}
		else if (parent->GetAcceptWebSockets())
		{
			...
			{
				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, 0, "TcpTransport accepted socket begin websocket handshake " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
				...
				boost::function<void(boost::string_ref protocol, const boost::system::error_code& ec)> h = boost::bind(&TcpAcceptor::AcceptSocket5, shared_from_this(),
					RR_BOOST_PLACEHOLDERS(_2), socket, websocket, socket_closer, handler);
				websocket->async_server_handshake("robotraconteur.robotraconteur.com", parent->GetWebSocketAllowedOrigins(), h);
				return;
			}
			else
			{
				ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, 0, "TcpTransport accepted socket invalid HTTP websocket header " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
		...
		{		
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, 0, "TcpTransport accepted socket invalid protocol " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
			RR_SHARED_PTR<ConnectionException> err = RR_MAKE_SHARED<ConnectionException>("Invalid protocol");
			handler(RR_SHARED_PTR<boost::asio::ip::tcp::socket>(), RR_SHARED_PTR<ITransportConnection>(), err);
		}
	}
	...
	void TcpAcceptor::AcceptSocket(RR_SHARED_PTR<boost::asio::ip::tcp::socket> socket, boost::function<void(RR_SHARED_PTR<boost::asio::ip::tcp::socket>, RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)>& handler)
	{
		...
		RR_SHARED_PTR<boost::signals2::scoped_connection> socket_closer
			= RR_MAKE_SHARED<boost::signals2::scoped_connection>(
				parent->AddCloseListener(socket, boost::bind(&boost::asio::ip::tcp::socket::close, RR_BOOST_PLACEHOLDERS(_1)))
				);
		...
		)
	{
		if (err)
		{
			...
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport websocket stream attach failed: " << err->what());
		...
		ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport connected websocket transport to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
		...
		)
	{
		...
		try
		{
			...
			boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpWebSocketConnector::Connect4, shared_from_this(),
				RR_BOOST_PLACEHOLDERS(_1), t, socket, websocket, boost::protect(handler));
			t->AsyncAttachWebSocket(socket, websocket, h);
			parent->AddCloseListener(t, &TcpTransportConnection::Close);
		}
		catch (std::exception& exp)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport websocket could not connect to supplied URL: " << exp.what());
		...
		boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		if (ec)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport websocket could not connect to supplied URL: " << ec);
		...
		try
		{
			...
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Websocket tcp socket connected, begin handshake");
			websocket->async_client_handshake(ws_url, "robotraconteur.robotraconteur.com",
				boost::bind(&TcpWebSocketConnector::Connect3, shared_from_this(), RR_BOOST_PLACEHOLDERS(_1), socket, websocket, websocket_closer, boost::protect(handler)));
	...
	void TcpWebSocketConnector::Connect(boost::string_ref url, uint32_t endpoint, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "TcpTransport begin websocket connection with URL: " << url);
		...
		try
		{
			...
			socket_connector->connect(ws_url, boost::bind(&TcpWebSocketConnector::Connect2, shared_from_this(),
				RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2), socket_connector, boost::protect(handler)));
			parent->AddCloseListener(socket_connector, &detail::websocket_tcp_connector::cancel);
		}
		catch (std::exception& exp)
		{
			ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, endpoint, "invalid websocket URL: " << url << " " << exp.what());
...
namespace detail {
		...
		)
	{
		if (err)
		{
			...
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss stream attach failed: " << err->what());
			handler(RR_SHARED_PTR<ITransportConnection>(), err);
			return;
		}
		...
		ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport connected wss transport to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
		handler(connection, RR_SHARED_PTR<RobotRaconteurException>());
	}
		...
		)
	{
		...
		if (ec)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss websocket handshake failed: " << ec);
			RR_SHARED_PTR<ConnectionException> err = RR_MAKE_SHARED<ConnectionException>("Could not connect to remote websocket");
			handler(RR_SHARED_PTR<ITransportConnection>(), err);
			return;
		}
		...
		try
		{
			...
			boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h= boost::bind(&TcpWSSWebSocketConnector::Connect4, shared_from_this(),
				RR_BOOST_PLACEHOLDERS(_1), t, socket, tls_stream, websocket, boost::protect(handler));
			t->AsyncAttachWSSWebSocket(socket, tls_stream, websocket, h);
			parent->AddCloseListener(t, &TcpTransportConnection::Close);
		}
		catch (std::exception& exp)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss could not connect to supplied URL: " << exp.what());
			handler(RR_SHARED_PTR<TcpTransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Could not connect to service"));
		}
		...
		boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		if (ec)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss HTTP TLS client handshake failed: " << ec.message());
			RR_SHARED_PTR<ConnectionException> err = RR_MAKE_SHARED<ConnectionException>("Could not contact remote host");
			handler(RR_SHARED_PTR<ITransportConnection>(), err);
			return;
		}
		...
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss could not verify server certificate \"" << servername << "\"");
		...
		try
		{
			...
			websocket->async_client_handshake(ws_url, "robotraconteur.robotraconteur.com",
				boost::bind(&TcpWSSWebSocketConnector::Connect3, shared_from_this(), RR_BOOST_PLACEHOLDERS(_1), socket, socket_closer, tls_stream, websocket, boost::protect(handler)));
		}
		catch (std::exception& exp)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss could not connect to supplied URL: " << exp.what());
		...
		boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "Websocket tcp socket connected, begin wss HTTP TLS handshake");
		try
		{
			...
			RR_SHARED_PTR<RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter>
				tls_stream = RR_MAKE_SHARED<RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter>(boost::ref(parent->GetNode()->GetThreadPool()->get_io_context()),
					...
					servername,
					boost::bind(TcpTransportConnection_socket_read_adapter, node, socket1, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
					boost::bind(TcpTransportConnection_socket_write_adapter, node, socket1, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
					boost::bind(TcpTransportConnection_socket_close_adapter, socket1)
				);

			RobotRaconteurNode::asio_async_handshake(node, tls_stream,boost::bind(&TcpWSSWebSocketConnector::Connect2_1, shared_from_this(), RR_BOOST_PLACEHOLDERS(_1), socket, socket_closer, tls_stream, boost::protect(handler)));
		}
		catch (std::exception& exp)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss HTTP TLS client handshake failed" << exp.what());
			RR_SHARED_PTR<ConnectionException> err = RR_MAKE_SHARED<ConnectionException>("Could not contact remote host");
			handler(RR_SHARED_PTR<ITransportConnection>(), err);
			return;
		}
	...
	void TcpWSSWebSocketConnector::Connect(boost::string_ref url, uint32_t endpoint, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "TcpTransport begin wss connection with URL: " << url);
			...
			)
		{
			...
			try
			{
				...
				RR_SHARED_PTR<TcpTransportConnection> t = RR_MAKE_SHARED<TcpTransportConnection>(parent, url, false, endpoint);
				boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpWSSWebSocketConnector::Connect4, shared_from_this(),
					RR_BOOST_PLACEHOLDERS(_1), t, socket, tls_stream, websocket, handler);
			...
			boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>) > handler)
		{
			...
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, endpoint, "wss tcp socket connected, begin wss HTTP TLS handshake");
			try
			{
				RR_SHARED_PTR<boost::signals2::scoped_connection> socket_closer
					= RR_MAKE_SHARED<boost::signals2::scoped_connection>(
...
#else
                context->set_verify_callback(boost::bind(&TcpWSSWebSocketConnector::verify_callback,RR_BOOST_PLACEHOLDERS(_1),RR_BOOST_PLACEHOLDERS(_2),servername));
#endif
				...
				RobotRaconteurNode::asio_async_handshake(node, tls_stream ,boost::asio::ssl::stream<boost::asio::ip::tcp::socket&>::client,
						boost::bind(&TcpWSSWebSocketConnector::Connect2_1, shared_from_this(), RR_BOOST_PLACEHOLDERS(_1), socket, socket_closer, tls_stream, boost::protect(handler)));
			}
			catch (std::exception& exp)
			{
				ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, endpoint, "TcpTransport wss HTTP TLS client handshake failed" << exp.what());
...
void TcpTransport::AsyncCreateTransportConnection(boost::string_ref url, RR_SHARED_PTR<Endpoint> e, boost::function<void (RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException> ) >& callback)
{
	...
	ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, e->GetLocalEndpoint(), "TcpTransport begin create transport connection with URL: " << url);

	{
		...
		if (max_connections > 0)
		{
			...
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, e->GetLocalEndpoint(), "Too many active TCP connections");
...
void TcpTransport::CloseTransportConnection(RR_SHARED_PTR<Endpoint> e)
{
	...
	ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, e->GetLocalEndpoint(), "TcpTransport request close transport connection");
...
void TcpTransport::StartServer(int32_t port)
{
	...
	if (port==48653) 
	{
		ROBOTRACONTEUR_LOG_ERROR_COMPONENT(node, Transport, -1, "TCP port 48653 is reserved for Robot Raconteur port sharer");
	...
	{
		ROBOTRACONTEUR_LOG_ERROR_COMPONENT(node, Transport, -1, "TcpTransport starting server on port : " << port << " failed: " << exp.what());
		throw;
	}
...
{
	ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "TcpTransport starting server using Robot Raconteur port sharer");
	boost::mutex::scoped_lock lock(port_sharer_client_lock);
	RR_SHARED_PTR<detail::TcpTransportPortSharerClient> c;
	if (port_sharer_client)
	{
	...
	ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, -1, "TcpTransport server started using Robot Raconteur port sharer");
}
...
void TcpTransport::handle_v4_accept(RR_SHARED_PTR<TcpTransport> parent,RR_SHARED_PTR<boost::asio::ip::tcp::acceptor> acceptor, RR_SHARED_PTR<boost::asio::ip::tcp::socket> socket, const boost::system::error_code& error)
{
	...
	try
	{
		...
		boost::function<void(RR_SHARED_PTR<boost::asio::ip::tcp::socket>, RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpTransport_connected_callback2, parent, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2), RR_BOOST_PLACEHOLDERS(_3));
	...
	{			
		...
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(parent->node, Transport, -1, "TcpTransport pausing IPv4 accept due to exceeding MaxConnectionCount");
...
void TcpTransport::handle_v6_accept(RR_SHARED_PTR<TcpTransport> parent, RR_SHARED_PTR<boost::asio::ip::tcp::acceptor> acceptor, RR_SHARED_PTR<boost::asio::ip::tcp::socket> socket, const boost::system::error_code& error)
{
	...
	{		
		if (connection_count > max_connection_count)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(parent->node, Transport, -1, "TcpTransport pausing IPv6 accept due to exceeding MaxConnectionCount");
...
#endif
		ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, -1, "Loaded TLS certificate for NodeID: " << GetNode()->NodeID().ToString());
	}
	catch (std::exception& exp)
	{
...
void TcpTransport::SetRequireTls(bool b)
{
	...
	ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "RequireTls set to: " << b);
}
...
void TcpTransport::EnableNodeDiscoveryListening(uint32_t flags)
{
	...
	ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "Node discovery listening enabled with flags 0x" << std::hex << flags);
...
bool TcpTransport::IsTransportConnectionSecure(uint32_t endpoint)
{
	RR_SHARED_PTR<ITransportConnection> t;
	{
		...
		if (e1 == TransportConnections.end()) 
		{
			ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, endpoint, "Transport connection to remote host not found");
			throw ConnectionException("Transport connection to remote host not found");
		}
		t = e1->second;
	}
...
bool TcpTransport::IsTransportConnectionSecure(RR_SHARED_PTR<Endpoint> endpoint)
{
	...
	RR_SHARED_PTR<ITransportConnection> t;			
	{
		...
		if (e1 == TransportConnections.end())
		{
			ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, endpoint->GetLocalEndpoint(), "Transport connection to remote host not found");
		 	throw ConnectionException("Transport connection to remote host not found");
		}
		t = e1->second;
	}
...
/*void TcpTransportConnection::do_starttls3(RR_SHARED_PTR<RobotRaconteurException> error, boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)>& callback)
{
	...
	AsyncPauseSend(boost::bind(&TcpTransportConnection::do_starttls1, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1),boost::protect(callback)));
}*/
...
void TcpTransportConnection::do_starttls1(const std::string& noden, const boost::system::error_code& error, boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)>& callback)
{
	if (error)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Could not initialize TLS connection: " << error.message());
	...
	try
	{
		...
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "Client requesting mutual certificate authentication");
		...
		lock.unlock();
		{
			...
			boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpTransportConnection::do_starttls2, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1), callback);
	...
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed sending STARTTLS: " << exp.what());
		{
			boost::mutex::scoped_lock lock(streamop_lock);
...
void TcpTransportConnection::do_starttls2(RR_SHARED_PTR<RobotRaconteurException> error, boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)>& callback)
{
	if (error)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed sending STARTTLS: " << error->what());
		{
			boost::mutex::scoped_lock lock(streamop_lock);
			starttls_handler.clear();
			starttls_timer.reset();
...
void TcpTransportConnection::do_starttls4(const std::string& servername, const boost::system::error_code& error)
{
	...
	if (ec2)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed receiving STARTTLS: " << ec2.message());
	...
	try
	{
		...
		if (!use_websocket && !use_wss_websocket)
		{
			...
			tls_socket = RR_MAKE_SHARED<RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter>(boost::ref(GetNode()->GetThreadPool()->get_io_context()),
				...
				RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter::client,
				servername,
				boost::bind(detail::TcpTransportConnection_socket_read_adapter, node, socket1, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
				boost::bind(detail::TcpTransportConnection_socket_write_adapter, node, socket1, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
				boost::bind(detail::TcpTransportConnection_socket_close_adapter, socket1)
				);
			RobotRaconteurNode::asio_async_handshake(node, tls_socket, boost::bind(&TcpTransportConnection::do_starttls5, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
		}
		else if (use_websocket)
		{
			...
			tls_websocket = RR_MAKE_SHARED<RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter>(boost::ref(GetNode()->GetThreadPool()->get_io_context()),
				...
				RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter::client,
				servername,
				boost::bind(detail::TcpTransportConnection_websocket_read_adapter, websocket.get(), RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
				boost::bind(detail::TcpTransportConnection_websocket_write_adapter, websocket.get(), RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
				boost::bind(detail::TcpTransportConnection_websocket_close_adapter, websocket.get())
				);
			RobotRaconteurNode::asio_async_handshake(node, tls_websocket, boost::bind(&TcpTransportConnection::do_starttls5, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
		...
		else
		{
			...
			tls_wss_websocket = RR_MAKE_SHARED<RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter>(boost::ref(GetNode()->GetThreadPool()->get_io_context()),
				...
				RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter::client,
				servername,
				boost::bind(detail::TcpTransportConnection_wss_websocket_read_adapter, wss_websocket.get(), RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
				boost::bind(detail::TcpTransportConnection_wss_websocket_write_adapter, wss_websocket.get(), RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)),
				boost::bind(detail::TcpTransportConnection_wss_websocket_close_adapter, wss_websocket.get())
				);
			RobotRaconteurNode::asio_async_handshake(node, tls_wss_websocket, boost::bind(&TcpTransportConnection::do_starttls5, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
		}
#endif
		...
		if (!use_websocket && !use_wss_websocket)
		{
			...
			RobotRaconteurNode::asio_async_handshake(node, tls_socket, boost::asio::ssl::stream<boost::asio::ip::tcp::socket&>::client, boost::bind(&TcpTransportConnection::do_starttls5, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
		}
		else if (use_websocket)
		{
			tls_context=RR_STATIC_POINTER_CAST<detail::OpenSSLAuthContext>(p->GetTlsContext());
			...
			RobotRaconteurNode::asio_async_handshake(node, tls_websocket, boost::asio::ssl::stream<detail::websocket_stream<boost::asio::ip::tcp::socket&> &>::client, boost::bind(&TcpTransportConnection::do_starttls5, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
		...
		else
		{
			...
			RobotRaconteurNode::asio_async_handshake(node, tls_wss_websocket, boost::asio::ssl::stream<detail::websocket_stream<boost::asio::ssl::stream<boost::asio::ip::tcp::socket&> &> &>::client, boost::bind(&TcpTransportConnection::do_starttls5, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
	...
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed starting TLS handshake: " << exp.what());
		boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> starttls_handler1 = starttls_handler;
		starttls_handler.clear();
		starttls_timer.reset();
		lock.unlock();
...
void TcpTransportConnection::do_starttls5(const boost::system::error_code& error)
{
	...
	if (error)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client TLS handshake failed: " << error.message());
		boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> starttls_handler1=starttls_handler;
		starttls_handler.clear();
		starttls_timer.reset();
		lock.unlock();
	...
	if (!use_websocket && !use_wss_websocket)
	{
		...
		{
			ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client could not verify server TLS certificate");
	...
	{				
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed resuming messages: " << exp.what());
		detail::InvokeHandlerWithException(node, starttls_handler1, RR_MAKE_SHARED<ConnectionException>("Could not initialize TLS connection"));
		return;
	}
	...
	try
	{
		...
		boost::function<void(RR_SHARED_PTR<RRObject>, RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpTransportConnection::do_starttls5_1, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2), starttls_handler1);
		AsyncStreamOp("CreateConnection", args, h);

		ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport TLS activated on client connection to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
		//starttls_handler1(RR_SHARED_PTR<RobotRaconteurException>());
		return;
	}
	catch(std::exception& e)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed CreateConnection after activating TLS: " << e.what());
...
void TcpTransportConnection::do_starttls5_1(RR_SHARED_PTR<RRObject> parameter, RR_SHARED_PTR<RobotRaconteurException> err, boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)>& callback)
{
	...
	if (err)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Client failed creating connection after activating TLS: " << err->what());
		try
		{
			Close();
		}
	...
	ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "CreateConnection complete after activating TLS");
...
void TcpTransportConnection::do_starttls6(const boost::system::error_code& error, RR_INTRUSIVE_PTR<Message> request)
{
	...
	boost::function<void(const boost::system::error_code&) > h = boost::bind(&TcpTransportConnection::do_starttls7, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1), request);
	AsyncPauseSend(h);
}
...
void TcpTransportConnection::do_starttls7(const boost::system::error_code& error, RR_INTRUSIVE_PTR<Message> request)
{
	...
	else
	{
		...
		if (tls_mutual_auth)
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "Server requesting mutual certificate authentication");
			mmret->AddElement("mutualauth", stringToRRArray("true"));
		}
	}
	mret->entries.push_back(mmret);
	boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpTransportConnection::do_starttls8, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1), request);
...
void TcpTransportConnection::do_starttls8(RR_SHARED_PTR<RobotRaconteurException> error, RR_INTRUSIVE_PTR<Message> request)
{
	if(error)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server failed sending STARTTLS: " << error->what());
		Close();
		return;
	}
	...
	if (!use_websocket && !use_wss_websocket)
	{
		...
		RobotRaconteurNode::asio_async_handshake(node, tls_socket, boost::bind(&TcpTransportConnection::do_starttls9, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()),RR_BOOST_PLACEHOLDERS(_1)));
	}
	else if (use_websocket)
	{
		tls_websocket = RR_MAKE_SHARED<RobotRaconteur::detail::TlsSchannelAsyncStreamAdapter>(boost::ref(GetNode()->GetThreadPool()->get_io_context()),
		...
		RobotRaconteurNode::asio_async_handshake(node, tls_websocket, boost::bind(&TcpTransportConnection::do_starttls9, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()),RR_BOOST_PLACEHOLDERS(_1)));
	...
	else
	{
		...
		RobotRaconteurNode::asio_async_handshake(node, tls_wss_websocket,boost::bind(&TcpTransportConnection::do_starttls9, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()),RR_BOOST_PLACEHOLDERS(_1)));
	...
	if (!use_websocket && !use_wss_websocket)
	{
		...
		RobotRaconteurNode::asio_async_handshake(node, tls_socket, boost::asio::ssl::stream<boost::asio::ip::tcp::socket&>::server, boost::bind(&TcpTransportConnection::do_starttls9, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()),RR_BOOST_PLACEHOLDERS(_1)));
	}
	else if (use_websocket)
	{
		tls_context=RR_STATIC_POINTER_CAST<detail::OpenSSLAuthContext>(p->GetTlsContext());
		...
		RobotRaconteurNode::asio_async_handshake(node, tls_websocket, boost::asio::ssl::stream<boost::asio::ip::tcp::socket&>::server, boost::bind(&TcpTransportConnection::do_starttls9, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()),RR_BOOST_PLACEHOLDERS(_1)));
	...
	else
	{
		...
		RobotRaconteurNode::asio_async_handshake(node, tls_wss_websocket, boost::asio::ssl::stream<detail::websocket_stream<boost::asio::ssl::stream<boost::asio::ip::tcp::socket&> &> &>::server, boost::bind(&TcpTransportConnection::do_starttls9, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()),RR_BOOST_PLACEHOLDERS(_1)));
	...
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server failed starting TLS handshake: " << exp.what());
	}
...
void TcpTransportConnection::do_starttls9(const boost::system::error_code& error)
{
	if(error)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server TLS handshake failed: " << error.message());
		Close();
		return;
	}
	...
	ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport TLS activated on server connection to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
	...
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server failed resuming messages: " << exp.what());
		Close();
		return;
	}
...
void TcpTransportConnection::MessageReceived(RR_INTRUSIVE_PTR<Message> m)
{
	...
	if (require_tls && !is_tls)
	{
		...
		if (bad_message)
		{
			ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, m_LocalEndpoint, "Message received without TLS active when TLS is required, closing transport");
	...
	if (is_tls)
	{
		if (!RemoteNodeID1.IsAnyNode())
		{
			if (RemoteNodeID1 != m->header->SenderNodeID)
			{
				...
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Message with invalid SenderNodeID received: " << m->header->SenderNodeID.ToString() << " expected: " << RemoteNodeID1.ToString());
				if (ret1->entries.size() >0)
				{
					boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpTransportConnection::SimpleAsyncEndSendMessage, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1));
		...
		if (local_ep!=0 && remote_ep!=0)
		{
			if (local_ep!=m->header->ReceiverEndpoint || remote_ep!=m->header->SenderEndpoint)
			{
				...
				if (remote_ep!=m->header->SenderEndpoint)
				{
					...
					ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Message received with invalid SenderEndpoint received: " << m->header->SenderEndpoint << " expected: " << remote_ep);
				}
				if (local_ep!=m->header->ReceiverEndpoint)
				{
					...
					ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Message received with invalid ReceiverEndpoint received: " << m->header->ReceiverEndpoint << " expected: " << local_ep);
				}
				if (ret1->entries.size() >0)
				{					
					boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h =  boost::bind(&TcpTransportConnection::SimpleAsyncEndSendMessage, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1));
	...
	if (ret != 0)
	{
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport sending special request response to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
		try
		{
			...
			if ((ret->entries.at(0)->EntryType == MessageEntryType_ConnectClientRet || ret->entries.at(0)->EntryType == MessageEntryType_ReconnectClient || ret->entries.at(0)->EntryType == MessageEntryType_ConnectClientCombinedRet) && ret->entries.at(0)->Error == MessageErrorType_None)
			{
				if (ret->entries.at(0)->Error == MessageErrorType_None)
				{
					if (ret->header->SenderNodeID == GetNode()->NodeID())
					{
						...
						ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport connection to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket) << " assigned LocalEndpoint: " << m_LocalEndpoint);
					}
				}
	...
	try
	{
		...
		if (m->entries.size()==1)
		{
			if ((m->entries[0]->EntryType == MessageEntryType_ConnectClientRet || m->entries[0]->EntryType == MessageEntryType_ConnectClientCombinedRet) && remote_ep==0)
			{
				...
				{
					ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport connection to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket) << " assigned RemoteEndpoint: " << m->header->SenderEndpoint);
		...
		if (is_tls)
		{
			if (local_ep == 0 || remote_ep == 0)
			{
				if (m->entries.size() != 1)
				{
					ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport connection invalid message before client connection created");
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport received message from " << TcpTransport_socket_local_endpoint(socket) << " passing to node");
		p->MessageReceived(m);
	}
	catch (std::exception& exp)
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport failed receiving message from " << TcpTransport_socket_local_endpoint(socket) << ": " << exp.what());
		RobotRaconteurNode::TryHandleException(node, &exp);
		Close();
	}
		...
		if (mm->MemberName == "STARTTLS")
		{
			if (mm->EntryType==MessageEntryType_StreamOp)
			{
					...
					|| (m->header->ReceiverNodeID == GetNode()->NodeID() && m->header->ReceiverNodeName==GetNode()->NodeName()))
				{
					...
					ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "Server received STARTTLS request from client");

					//We have a match!
					boost::function<void(const boost::system::error_code&)> h = boost::bind(&TcpTransportConnection::do_starttls6, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1), m);
					AsyncPauseReceive(h);
					return;
				}
				else
				{
					...
					ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server requested an invalid STARTTLS request");
					...
					boost::function<void(RR_SHARED_PTR<RobotRaconteurException>)> h = boost::bind(&TcpTransportConnection::StreamOp_EndSendMessage, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1));
			...
			if (mm->EntryType==MessageEntryType_StreamOpRet)
			{
				...
				{					
					...
					ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server returned an error during STARTTLS request: " << ee->what());
				...
				{
					ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Server returned an invalid STARTTLS response");
				...
				if (mm->TryFindElement("mutualauth", e))
				{
					...
					if (dat == "true")
					{
						ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "Mutual certificate authentication activated");
						tls_mutual_auth = true;
					}
				}				

				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, m_LocalEndpoint, "Client received success STARTTLS from server");

				boost::function<void(const boost::system::error_code&)> h = boost::bind(&TcpTransportConnection::do_starttls4, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), servername, RR_BOOST_PLACEHOLDERS(_1));
				AsyncPauseReceive(h);
				return;
			}
		}
...
void TcpTransportConnection::Close()
{
	...
	closing=true;
	{
		ROBOTRACONTEUR_LOG_INFO_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport closing transport connection to " << TcpTransport_socket_remote_endpoint(socket) << " from " << TcpTransport_socket_local_endpoint(socket));
		boost::mutex::scoped_lock lock(socket_lock);
		...
		if (false /*is_tls*/)
		{
			try
			{
				if (!use_websocket && !use_wss_websocket)
				{
					RobotRaconteurNode::asio_async_shutdown(node, tls_socket, boost::bind(&TcpTransportConnection::Close1, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
				}
				else if (use_websocket)
				{
					RobotRaconteurNode::asio_async_shutdown(node, tls_websocket, boost::bind(&TcpTransportConnection::Close1, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
				}
				else 
				{
					RobotRaconteurNode::asio_async_shutdown(node, tls_wss_websocket, boost::bind(&TcpTransportConnection::Close1, RR_STATIC_POINTER_CAST<TcpTransportConnection>(shared_from_this()), RR_BOOST_PLACEHOLDERS(_1)));
				}
			}
			catch (std::exception&)
			{
...
void TcpTransportConnection::ForceClose()
{
	...
	ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, m_LocalEndpoint, "TcpTransport force closing transport connection");
...
namespace detail
{
	...
	{
		{
			...
			if (listening)
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport discovery already listening for nodes");
	...
	void IPNodeDiscovery::handle_receive(const boost::system::error_code& error, std::size_t bytes_transferred, RR_SHARED_PTR<boost::asio::ip::udp::socket> socket, RR_SHARED_PTR<boost::asio::ip::udp::endpoint> ep, boost::shared_array<uint8_t> buffer)
	{
		...
		if (!error)
		{
			ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "TcpTransport discovery received packet length " << bytes_transferred << " from " << *ep);
			...
			{
				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "TcpTransport discovery error processing node announce packet: " << exp.what());
			}
		}
		...
		if (broadcasting)
		{
			try
			{
				...
				if (packet.substr(0, seed.length()) == seed)
				{
					...
					{
						ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "TcpTransport discovery received discovery request, sending announce now");
						SendAnnounceNow();
					}
			...
			BOOST_FOREACH (const boost::asio::ip::address& e, local_addresses)
			{
				BOOST_FOREACH (std::string& ee, tcpschemes)
				{
					...
					try
					{
						...
						ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Transport, -1, "TcpTransport discovery sent broadcast packet to " << e << " \"" 
							<< boost::replace_all_copy(packetdata, "\n", "\\n"));
						
					}
					catch (std::exception& exp)
					{
						ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport discovery failed sending broadcast packet: " << exp.what());
					}
	...
	{		
		...
		BOOST_FOREACH(const boost::asio::ip::address& e, local_addresses)
		{
			...
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport discovery failed sending discovery request packet: " << exp.what());
			}
		}
	...
	void TcpTransportPortSharerClient::client_thread()
	{
		...
		while (true)
		{
			...
			try
			{
				...
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport port sharer connecting named pipe to: \"" << fname << "\"");
				...
				while (true)
				{
					...
					{
						ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport port sharer socket handoff failed: " << exp2.what());
					}
#else
			...
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport port sharer connect failed: " << exp.what());
			}
	...
	{
		ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Transport, -1, "TcpTransport port sharer accepted socket from " << TcpTransport_socket_remote_endpoint(socket) << " to " << TcpTransport_socket_local_endpoint(socket));
		...
		try
		{
			...
			//TcpTransport_attach_transport(parent1, socket, "", true, 0, boost::bind(&TcpTransport_connected_callback2, parent1, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2), RR_BOOST_PLACEHOLDERS(_3)));
			...
			boost::function<void(RR_SHARED_PTR<boost::asio::ip::tcp::socket>, RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> h 
				= boost::bind(&TcpTransport_connected_callback2, parent1, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2), RR_BOOST_PLACEHOLDERS(_3));
			a->AcceptSocket(socket, h);
		}
		catch (std::exception& exp)
		{
