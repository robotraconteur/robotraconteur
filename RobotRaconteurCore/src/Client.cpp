...
namespace RobotRaconteur
{
	...
	RR_SHARED_PTR<RRObject> ClientContext::FindObjRef(boost::string_ref path, boost::string_ref objecttype2)
	{
		...
		AsyncFindObjRef(path, objecttype2, boost::bind(&detail::sync_async_handler<RRObject>::operator(), h, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
		return h->end();
	}
	...
	std::string ClientContext::FindObjectType(boost::string_ref path)
	{
		...
		AsyncFindObjectType(path, boost::bind(&detail::sync_async_handler<std::string>::operator(), h, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
		return *h->end();
	}
	...
	RR_INTRUSIVE_PTR<MessageEntry> ClientContext::ProcessRequest(RR_INTRUSIVE_PTR<MessageEntry> m)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node, Client, GetLocalEndpoint(), m->ServicePath, m->MemberName, "ProcessRequest sending message with requestid " << myrequestid << " EntryType " << m->EntryType);
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node, Client, GetLocalEndpoint(), m->ServicePath, m->MemberName, "ProcessRequest received response with requestid " << myrequestid);

		}
		catch (std::exception& exp)
		{
			ROBOTRACONTEUR_LOG_DEBUG_COMPONENT_PATH(node, Client, GetLocalEndpoint(), m->ServicePath, m->MemberName, "Error during ProcessRequest: " << exp.what());
			throw;
		}
		...
		if (rec_message->Error != MessageErrorType_None)
		{
			...
			{
				ROBOTRACONTEUR_LOG_DEBUG_COMPONENT_PATH(node, Client, GetLocalEndpoint(), m->ServicePath, m->MemberName, "Service returned error during ProcessRequest: " << rec_message->Error);
			}
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node, Client, GetLocalEndpoint(), m->ServicePath, m->MemberName, "ProcessRequest with requestid " << myrequestid << " successful");
	...
	RR_SHARED_PTR<RRObject> ClientContext::ConnectService(RR_SHARED_PTR<Transport> c, boost::string_ref url, boost::string_ref username, RR_INTRUSIVE_PTR<RRMap<std::string, RRValue> > credentials, boost::string_ref objecttype)
	{
		...
		AsyncConnectService(c, url, username, credentials, objecttype, boost::bind(&detail::sync_async_handler<RRObject>::operator(), d, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
		return d->end();
	}
	...
	RR_SHARED_PTR<RRObject> ClientContext::ConnectService(RR_SHARED_PTR<Transport> c, RR_SHARED_PTR<ITransportConnection> tc, boost::string_ref url, boost::string_ref username, RR_INTRUSIVE_PTR<RRMap<std::string, RRValue> > credentials, boost::string_ref objecttype)
	{
		...
		RR_SHARED_PTR<detail::sync_async_handler<RRObject> > d = RR_MAKE_SHARED<detail::sync_async_handler<RRObject> >(RR_MAKE_SHARED<ConnectionException>("Connection timed out"));
		AsyncConnectService(c, tc, url, username, credentials, objecttype, boost::bind(&detail::sync_async_handler<RRObject>::operator(), d, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
		return d->end();
	}
	...
	PullServiceDefinitionReturn ClientContext::PullServiceDefinition(boost::string_ref ServiceType)
	{
		...
		AsyncPullServiceDefinition(ServiceType, boost::bind(&detail::sync_async_handler<PullServiceDefinitionReturn>::operator(), d, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
	...
	PullServiceDefinitionAndImportsReturn ClientContext::PullServiceDefinitionAndImports(boost::string_ref servicetype)
	{
		...
		AsyncPullServiceDefinitionAndImports(servicetype, boost::bind(&detail::sync_async_handler<PullServiceDefinitionAndImportsReturn>::operator(), d, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
	...
	std::string ClientContext::AuthenticateUser(boost::string_ref username, RR_INTRUSIVE_PTR<RRValue> credentials)
	{
		...
		AsyncAuthenticateUser(username, credentials, boost::bind(&detail::sync_async_handler<std::string>::operator(), d, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)));
		return *d->end();
	}
	...
	std::string ClientContext::RequestObjectLock(RR_SHARED_PTR<RRObject> obj, RobotRaconteurObjectLockFlags flags)
	{
		...
		AsyncRequestObjectLock(obj, flags, boost::bind(&detail::sync_async_handler<std::string>::operator(), t, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
		return *t->end();
	}
	...
	std::string ClientContext::ReleaseObjectLock(RR_SHARED_PTR<RRObject> obj)
	{
		...
		RR_SHARED_PTR<detail::sync_async_handler<std::string> > t = RR_MAKE_SHARED<detail::sync_async_handler<std::string> >();
		AsyncReleaseObjectLock(obj, boost::bind(&detail::sync_async_handler<std::string>::operator(), t, RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2)), GetNode()->GetRequestTimeout());
		return *t->end();
	}
	...
	std::string ClientContext::MonitorEnter(RR_SHARED_PTR<RRObject> obj, int32_t timeout)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Client, GetLocalEndpoint(), "Begin MonitorEnter for servicepath \"" << stub2->ServicePath << "\"");
...

		{
			...
			{
				iserror = false;
				ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Client, GetLocalEndpoint(), "MonitorEnter for successful for servicepath \"" << s->ServicePath << "\"");
				return "OK";
			...
			if (retcode == "Continue")
			{
				while (keep_trying)
				{
					ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Client, GetLocalEndpoint(), "MonitorEnter continue enter for servicepath \"" << s->ServicePath << "\"");
	...
	void ClientContext::MonitorExit(RR_SHARED_PTR<RRObject> obj)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Client, GetLocalEndpoint(), "Begin MonitorExit for servicepath \"" << stub2->ServicePath << "\"");
