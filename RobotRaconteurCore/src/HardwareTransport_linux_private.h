...
typedef DBusMessage * (*dbus_connection_send_with_reply_and_block_t) (DBusConnection *connection, DBusMessage *message, int timeout_milliseconds, DBusError *error);
typedef void (*dbus_connection_unref_t) (DBusConnection *connection);
typedef DBusMessage* (*dbus_message_new_method_call_t) (const char  *bus_name, const char  *path, const char  *iface, const char  *method);
...
typedef int (*sdp_service_search_attr_req_t) (sdp_session_t *session, const sdp_list_t *search, sdp_attrreq_type_t reqtype, const sdp_list_t *attrid_list, sdp_list_t **rsp_list);
...
{
public:
	BluezBluetoothConnector(RR_SHARED_PTR<HardwareTransport> parent, RR_SHARED_PTR<void> dbus_f, RR_SHARED_PTR<void> sdp_f);
...
class HardwareTransport_linux_discovery : public HardwareTransport_discovery<LibUsbDeviceManager, BluezBluetoothConnector>
{
public:

	HardwareTransport_linux_discovery(RR_SHARED_PTR<HardwareTransport> parent, const std::vector<std::string>& schemes, RR_SHARED_PTR<LibUsbDeviceManager> usb, RR_SHARED_PTR<BluezBluetoothConnector> bt);
