...
class Discovery : public RR_ENABLE_SHARED_FROM_THIS<Discovery>
{
  ...
  protected:
    ...
    void EndUpdateServiceInfo(RR_SHARED_PTR<Discovery_nodestorage> storage,
    ...
    void DoSubscribe(const std::vector<std::string>& service_types, RR_SHARED_PTR<ServiceSubscriptionFilter> filter,
                     RR_SHARED_PTR<IServiceSubscription> subscription);
};
...
class Discovery_findservicebytype : public RR_ENABLE_SHARED_FROM_THIS<Discovery_findservicebytype>
{
  ...
  public:
    ...
    void serviceinfo_callback(RR_INTRUSIVE_PTR<MessageEntry> ret1, RR_SHARED_PTR<RobotRaconteurException> err,
                              RR_SHARED_PTR<ServiceStub> client, std::string url, uint32_t key);

    void connect_callback(RR_SHARED_PTR<RRObject> client, RR_SHARED_PTR<RobotRaconteurException> err, std::string url,
                          uint32_t key);

    void find2();

    void AsyncFindServiceByType(boost::string_ref servicetype, const std::vector<std::string>& schemes,
                                RR_MOVE_ARG(boost::function<void(RR_SHARED_PTR<std::vector<ServiceInfo2> >)>) handler,
                                int32_t timeout);
};
