...
namespace RobotRaconteur
{
	...
	void AutoResetEvent::WaitOne()
	{
		...
		if (waitres==WAIT_OBJECT_0 || waitres==WAIT_ABANDONED) return;
	...
	bool AutoResetEvent::WaitOne(int32_t timeout)
	{
		...
		if (waitres==WAIT_OBJECT_0) return true;
		
		if (waitres==WAIT_TIMEOUT || waitres==WAIT_ABANDONED) return false;
	...
	AutoResetEvent::AutoResetEvent() : m_mutex()
	{
		m_bSet=false;
	}
	...
	bool AutoResetEvent::WaitOne(int32_t timeout)
	{
		...
		boost::system_time const timeout1=boost::posix_time::microsec_clock::universal_time() + boost::posix_time::milliseconds(timeout);
		while (!m_bSet)
		{
			boost::unique_lock<boost::mutex> lock(m_mutex);
			m_setCondition.timed_wait(lock,timeout1);
			if ( !m_bSet && boost::posix_time::microsec_clock::universal_time() > timeout1) return false;
