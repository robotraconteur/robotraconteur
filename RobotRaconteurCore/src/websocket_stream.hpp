...
public:
	...
	void async_server_handshake(std::string protocol, std::vector<std::string> allowed_origins, boost::function<void(const std::string& protocol, const boost::system::error_code& ec)> handler)
	{
...
protected:
	...
	void server_handshake2(boost::shared_array<uint8_t> buf, std::string protocol, std::vector<std::string> allowed_origins, std::size_t n, const boost::system::error_code& ec, boost::function<void(const std::string& protocol, const boost::system::error_code& ec)> handler)
	{
		if (ec)
		{
			{
		...
		while (pos < boost::numeric_cast<int64_t>(n) && !end_header)
		{
			...
			else
			{
				...
				if (name == "Sec-WebSocket-Version" && handshake_recv_args.find("Sec-WebSocket-Version") != handshake_recv_args.end())
				{
					std::string value2 = handshake_recv_args.at("Sec-WebSocket-Version") + ", " + value;
					handshake_recv_args.at("Sec-WebSocket-Version") = value2;
				}
				if (name == "Sec-WebSocket-Protocol" && handshake_recv_args.find("Sec-WebSocket-Protocol") != handshake_recv_args.end())
				{
		...
		if (handshake_recv_args.find("Upgrade") == handshake_recv_args.end() || handshake_recv_args.find("Sec-WebSocket-Key") == handshake_recv_args.end() || handshake_recv_args.find("Sec-WebSocket-Version") == handshake_recv_args.end())
		{
		...
		if (handshake_recv_args.find("Origin") != handshake_recv_args.end() || handshake_recv_args.find("origin") != handshake_recv_args.end())
		{
			...
			if (!good_origin)
			{
				...
				BOOST_FOREACH (std::string& e, allowed_origins)
				{
					if (!boost::contains(e,"*"))
					{
						if (e == origin)
	...
	void send_server_error(std::string error, boost::function<void(const std::string& protocol, const boost::system::error_code& ec)> handler)
	{
		...
		boost::mutex::scoped_lock lock(next_layer_lock);
		next_layer_.async_write_some(boost::asio::buffer(data->c_str(), data->size()), boost::bind(&websocket_stream::end_send_server_error, this, data, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, boost::protect(handler)));
		
	}

	void end_send_server_error(boost::shared_ptr<std::string> data, std::size_t n, const boost::system::error_code& ec, boost::function<void(const std::string& protocol, const boost::system::error_code& ec)> handler)
	{
		...
		boost::mutex::scoped_lock lock(next_layer_lock);
		next_layer_.async_write_some(boost::asio::buffer(data2->c_str(), data2->size()), boost::bind(&websocket_stream::end_send_server_error, this, data2, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, boost::protect(handler)));
	}

	void send_server_success_response(std::string accept_key, std::string protocol, boost::function<void(const std::string& protocol, const boost::system::error_code& ec)> handler)
	{
		std::string l = std::string("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: ") + accept_key + std::string("\r\nSec-WebSocket-Protocol: robotraconteur.robotraconteur.com\r\nSec-WebSocket-Version: 13\r\n\r\n");
		boost::shared_ptr<std::string> data = boost::make_shared<std::string>(l);
		boost::mutex::scoped_lock lock(next_layer_lock);
		next_layer_.async_write_some(boost::asio::buffer(data->c_str(), data->size()), boost::bind(&websocket_stream::end_send_server_success_response, this, data, protocol, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, boost::protect(handler)));

	}

	void end_send_server_success_response(boost::shared_ptr<std::string> data, std::string protocol, std::size_t n, const boost::system::error_code& ec, boost::function<void(const std::string& protocol, const boost::system::error_code& ec)> handler)
	{
	...
	public:
	void async_client_handshake(const std::string& url, const std::string& protocol, boost::function<void(const boost::system::error_code& ec)> handler)
	{
		...
		boost::regex_search(url, url_result, boost::regex("^([^:]+)://((?:\\[[A-Fa-f0-9:]+(?:\\%\\w*)?\\])|(?:[^\\[\\]\\:/\\?]+))(?::([^:/\\?]+))?/?(.*)$"));

		if (url_result.size()<4) throw InvalidArgumentException("Malformed URL");

		if (url_result[1] != "ws" && url_result[1] != "wss") throw InvalidArgumentException("Invalid transport type for websocket");
		...
		std::string data1 = "GET " + path + " HTTP/1.1\r\n"
			"Host: " + host + "\r\n"
			"Upgrade: websocket\r\n"
			"Connection: Upgrade\r\n"
		...
		boost::mutex::scoped_lock lock(next_layer_lock);
		next_layer_.async_write_some(boost::asio::buffer(data->c_str(), data->size()), boost::bind(&websocket_stream::async_client_handshake2, this,
			boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, data, url, protocol, key, boost::protect(handler)));


	}
	protected:

		void async_client_handshake2(const boost::system::error_code& ec, size_t n, boost::shared_ptr<std::string>(data), std::string url, std::string protocol, std::string key, boost::function<void(const boost::system::error_code& ec)> handler)
		{
			if (ec || n == 0)
			{
				{
			...
			if (n < data->size())
			{
				boost::shared_ptr<std::string> data2 = boost::make_shared<std::string>(data->substr(n));
				boost::mutex::scoped_lock lock(next_layer_lock);
				next_layer_.async_write_some(boost::asio::buffer(data2->c_str(), data2->size()), boost::bind(&websocket_stream::async_client_handshake2, this,
					boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, data, url, protocol, key, boost::protect(handler)));
				return;
			}
		...
		void async_client_handshake3(boost::shared_array<uint8_t> buf, const boost::system::error_code& ec, std::size_t n, std::string url, std::string protocol, std::string key, boost::function<void(const boost::system::error_code& ec)> handler)
		{
			if (ec)
			{
				{
			...
			if (boost::to_lower_copy(handshake_recv_args.at("Upgrade")) != "websocket" || boost::to_lower_copy(handshake_recv_args.at("Connection")) != "upgrade"
			...
			if (handshake_recv_args.find("Sec-WebSocket-Protocol") != handshake_recv_args.end())
			{
				if (boost::to_lower_copy(handshake_recv_args.at("Sec-WebSocket-Protocol")) != boost::to_lower_copy(protocol))
				{
					{
						boost::mutex::scoped_lock lock(next_layer_lock);
						next_layer_.lowest_layer().close();
	...
	std::string sha1_hash(std::string blob)
	{
	...
	void async_write_message(uint8_t command, const const_buffers& buffer,  boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{	
		...
		if (send_en_mask)
		{
			...
			buffer2.reset(new uint8_t[count]);
			boost::asio::buffers_iterator<const_buffers, uint8_t> buffer_iter= boost::asio::buffers_iterator<const_buffers, uint8_t>::begin(buffer);
		...
		next_layer_.async_write_some(send_buffer, boost::bind(&websocket_stream::async_write_message3, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, count, header, buffer2, boost::protect(handler)));

	}

	void async_write_message3(size_t n, const boost::system::error_code ec, size_t data_count, boost::shared_array<uint8_t> header_buf, boost::shared_array<uint8_t> buffer2, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{		
		if (ec || n == 0)
		{
		...
		{			
			...
			boost::mutex::scoped_lock lock(next_layer_lock);
			next_layer_.async_write_some(async_write_buffers, boost::bind(&websocket_stream::async_write_message3, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, data_count, header_buf, buffer2, boost::protect(handler)));
			return;				
		}
	...
	void async_write_message4(const boost::system::error_code& ec, size_t n, boost::shared_array<uint8_t> ping_data2, size_t ping_data_len2, size_t ping_data_pos2, boost::asio::const_buffer buffer, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{
	...
	uint8_t recv_header1[2];
	uint8_t recv_header2[12];

	uint64_t recv_frame_length;
	uint64_t recv_frame_pos;
	bool recv_frame_en_mask;
	uint8_t recv_frame_mask[4];
	uint8_t recv_frame_opcode;


	void async_read_some2(boost::asio::mutable_buffer buf, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{
		...
		boost::mutex::scoped_lock lock(next_layer_lock);
		next_layer_.async_read_some(boost::asio::buffer(recv_header1, 2), boost::bind(&websocket_stream::async_read_some3, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, buf, 0, boost::protect(handler)));

	}

	void async_read_some3(size_t n, const boost::system::error_code& ec, boost::asio::mutable_buffer buf, size_t pos, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{
		if (ec || n == 0)
		{
			handler(ec, 0);
		...
		if (n < 2 - pos)
		{
			...
			size_t pos2 = pos + n;
			boost::mutex::scoped_lock lock(next_layer_lock);
			next_layer_.async_read_some(boost::asio::buffer(recv_header1 + pos2, 2 - pos2), boost::bind(&websocket_stream::async_read_some3, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, buf, pos2, boost::protect(handler)));
			return;
		}
		...
		if (count1 == 126) header2_len += 2;
		if (count1 == 127) header2_len += 8;

		if (recv_en_mask1) header2_len += 4;
		...
		{
			boost::mutex::scoped_lock lock(next_layer_lock);
			next_layer_.async_read_some(boost::asio::buffer(recv_header2, header2_len), boost::bind(&websocket_stream::async_read_some4, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, opcode_recv1, recv_en_mask1, count1, buf, 0, boost::protect(handler)));
			return;
		}
		else
		{
	...
	void async_read_some4(size_t n, const boost::system::error_code& ec, uint8_t opcode, bool en_mask, uint8_t count1, boost::asio::mutable_buffer buf, size_t pos, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{
		if (ec || n == 0)
		{
			handler(ec, 0);
		...
		if (en_mask) header2_len += 4;

		if (n < header2_len - pos)
		{
			size_t pos2 = pos + n;
			boost::mutex::scoped_lock lock(next_layer_lock);
			next_layer_.async_read_some(boost::asio::buffer(recv_header2+pos2, header2_len-pos2), boost::bind(&websocket_stream::async_read_some4, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, opcode, en_mask, count1, buf, pos2, boost::protect(handler)));
			return;
		}
		...
		if (count1 < 126)
		{
			...
			if (en_mask)
			{
				memcpy(recv_frame_mask, recv_header2, 4);
			}
		}
		else if (count1 == 126)
		{
			...
			if (en_mask)
			{
				memcpy(recv_frame_mask, recv_header2 + 2, 4);
			}
		}
		else
		{
			...
			if (en_mask)
			{
				memcpy(recv_frame_mask, recv_header2 + 8, 4);
			}
		}
	...
	void async_read_some5(size_t n, const boost::system::error_code& ec, boost::asio::mutable_buffer buf, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{
	...
	void async_read_some6(size_t n, const boost::system::error_code& ec, boost::shared_array<uint8_t> op_buf, size_t op_len, size_t op_pos, boost::asio::mutable_buffer buf, boost::function<void(const boost::system::error_code&, std::size_t)> handler)
	{
		if (ec || (n == 0 && op_len != 0))
		{
			if (handler)
		...
		if (n < op_len - op_pos)
		{
			...
			boost::mutex::scoped_lock lock(next_layer_lock);
			next_layer_.async_read_some(boost::asio::buffer(op_buf.get()+op_pos2, op_len-op_pos2), boost::bind(&websocket_stream::async_read_some6, this, boost::asio::placeholders::bytes_transferred, boost::asio::placeholders::error, op_buf, op_len, op_pos2, buf, boost::protect(handler)));
	...
	public:
		...
		{
			if(boost::asio::buffer_size(boost::asio::detail::buffer_sequence_adapter<boost::asio::mutable_buffer, MutableBufferSequence >::first(buffers))==0)
			{
...
public:
	...
	void connect(const std::string& url, boost::function<void (const boost::system::error_code&, boost::shared_ptr<boost::asio::ip::tcp::socket> socket)> handler, int32_t timeout=10000)
	{
		boost::smatch url_result;
		boost::regex_search(url, url_result, boost::regex("^([^:]+)://((?:\\[[A-Fa-f0-9:]+(?:\\%\\w*)?\\])|(?:[^\\[\\]\\:/]+))(?::([^:/\\?]+))?/?(.*)$"));
		...
		if (port_str.size() == 0)
		{
			...
			if (url_result[1] == "ws") port_str = "80";
			if (url_result[1] == "wss") port_str = "443";
		}
		path = url_result[4];
...
protected:
	...
	void connect2(const boost::system::error_code& err, boost::asio::ip::tcp::resolver::results_type results, boost::function<void(const boost::system::error_code&, boost::shared_ptr<boost::asio::ip::tcp::socket> socket)> handler)
	{
		...
		if (err)
		{
			...
			lock.unlock();		
			handler(err, boost::shared_ptr<boost::asio::ip::tcp::socket>());
			return;
		}
		...
		if (cancelled)
		{
			...
			if (timeout_timer) timeout_timer->cancel();
			lock.unlock();
			handler(boost::asio::error::operation_aborted, boost::shared_ptr<boost::asio::ip::tcp::socket>());
			return;
		}
		...
		{ 
			...
			handler(ec1, boost::shared_ptr<boost::asio::ip::tcp::socket>());
			return;
		}
		...
		for (; endpoint_iterator != end; endpoint_iterator++)
		{
			...
			if (endpoint_iterator->endpoint().address().is_v6()) ipv6_1.push_back(endpoint_iterator->endpoint());
		}
		...
		BOOST_FOREACH (boost::asio::ip::tcp::endpoint& e, ipv4)
		{
			...
			k++;
			sock->async_connect(e, boost::bind(&websocket_tcp_connector::connect3, shared_from_this(), sock, boost::asio::placeholders::error, boost::protect(handler)));
		}
	...
	void connect3(boost::shared_ptr<boost::asio::ip::tcp::socket> sock, const boost::system::error_code& ec, boost::function<void(const boost::system::error_code&, boost::shared_ptr<boost::asio::ip::tcp::socket> socket)> handler)
	{
		...
		for (std::vector<boost::weak_ptr<boost::asio::ip::tcp::socket> >::iterator e = sockets_.begin(); e != sockets_.end();)
		{			
			if (e->expired())
			{
				e++;
		...
		if (ec)
		{
			if (sockets_.size() == 0 && !complete)
			{
				complete = true;
				if (timeout_timer) timeout_timer->cancel();
				lock.unlock();
				handler(ec, boost::shared_ptr<boost::asio::ip::tcp::socket>());
				return;
			}
			return;
		}
	...
	void connect4(const boost::system::error_code& ec, boost::function<void(const boost::system::error_code&, boost::shared_ptr<boost::asio::ip::tcp::socket> socket)> handler)
	{
		boost::mutex::scoped_lock lock(handler_lock);
		if (complete) return;
		complete = true;
