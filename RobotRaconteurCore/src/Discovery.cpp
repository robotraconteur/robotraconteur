                           ...
                           const RobotRaconteurServiceIndex::NodeInfo& ninfo)
{
    ...
    {
        BOOST_FOREACH (RR_INTRUSIVE_PTR<RobotRaconteur::RRArray<char> >& e,
                       *info.RootObjectImplements | boost::adaptors::map_values)
        {
    ...
    {
        BOOST_FOREACH (RR_INTRUSIVE_PTR<RobotRaconteur::RRArray<char> >& e,
                       *info.ConnectionURL | boost::adaptors::map_values)
        {
...
void Discovery_updatediscoverednodes::timeout_timer_callback(const TimerEvent& e)
{
    ...
    if (!e.stopped)
    {
        ...
        ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                           "UpdateDiscoveredNodes timed out, returning current results");
...
void Discovery_updatediscoverednodes::UpdateDiscoveredNodes(const std::vector<std::string>& schemes,
                                                            ...
                                                            int32_t timeout)
{
...

    {
        ...
        BOOST_FOREACH (const RR_SHARED_PTR<Transport>& e, transports)
        {
            ...
            {
                ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                   "Begin GetDetectedNodes failed for transport "
                                                       << e->GetUrlSchemeString() << ": " << exp.what());
            }
...
void Discovery_findservicebytype::handle_error(const int32_t& key, const RR_SHARED_PTR<RobotRaconteurException>& err)
{
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "FindServiceByType last candidate failed, returning " << ret->size()
                                                                                             << " discovered services");
...
void Discovery_findservicebytype::timeout_timer_callback(const TimerEvent& e)
{
    ...
    if (!e.stopped)
    {
        ...
        ROBOTRACONTEUR_LOG_TRACE_COMPONENT(
            node, Discovery, -1, "FindServiceByType timed out, returning " << ret->size() << " discovered services");
        detail::InvokeHandler(node, handler, ret);
    }
}
                                                       ...
                                                       int32_t key)
{
    ...
    if (err)
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(
            node, Discovery, -1, "FindServiceByType getting ServiceInfo2 from " << url << " failed: " << err->what());
    ...
    else
    {
        try
        {
            RR_SHARED_PTR<RobotRaconteurServiceIndex::NodeInfo> n =
                RR_MAKE_SHARED<RobotRaconteurServiceIndex::NodeInfo>();
            n->NodeID =
                ArrayToRRArray<uint8_t>(rr_cast<ServiceStub>(client)->GetContext()->GetRemoteNodeID().ToByteArray());
            ...
            if (ret1->Error == RobotRaconteur::MessageErrorType_None)
            {
                ...
                RR_INTRUSIVE_PTR<RobotRaconteur::RRMap<int32_t, RobotRaconteurServiceIndex::ServiceInfo> > ret =
                    RobotRaconteur::rr_cast<RobotRaconteur::RRMap<int32_t, RobotRaconteurServiceIndex::ServiceInfo> >(
                        (node->UnpackMapType<int32_t, RobotRaconteurServiceIndex::ServiceInfo>(
                            me->CastData<RobotRaconteur::MessageElementNestedElementList>())));

                if (ret)
                {
                    BOOST_FOREACH (RR_INTRUSIVE_PTR<RobotRaconteurServiceIndex::ServiceInfo>& ii,
                                   *ret | boost::adaptors::map_values)
                    {
                        if (!ii)
                            continue;
                        ...
                        {
                            BOOST_FOREACH (RR_INTRUSIVE_PTR<RRArray<char> >& impl,
                                           *ii->RootObjectImplements | boost::adaptors::map_values)
                            {
                ...
                ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                                   "FindServiceByType getting ServiceInfo2 from "
            ...
            if (done)
            {
                ...
                if (c2)
                {
                    ...
                    boost::mutex::scoped_lock lock(ret_lock);
                    ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                       "FindServiceByType completed successfully with "
                                                           << this->ret->size() << " discovered services");
        ...
        catch (std::exception& err2)
        {
            ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                               "FindServiceByType getting ServiceInfo2 from "
                                                   << url << " failed: " << err2.what());
            handle_error(key,
                         RobotRaconteurExceptionUtil::ExceptionToSharedPtr(err2, MessageErrorType_ConnectionError));
        }
    }
}
                                                   ...
                                                   const std::string& url, int32_t key)
{
    ...
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "FindServiceByType connecting to " << url << " failed: " << err->what());
        handle_error(key, err);
        return;
    }
    ...
    else
    {
        try
        {
            ...
            int32_t key2 = 0;
            {
                ...
                RR_INTRUSIVE_PTR<RobotRaconteur::MessageEntry> rr_req = RobotRaconteur::CreateMessageEntry(
                    RobotRaconteur::MessageEntryType_FunctionCallReq, "GetLocalNodeServices");
                client3->AsyncProcessRequest(rr_req,
                                             boost::bind(&Discovery_findservicebytype::serviceinfo_callback,
                                                         shared_from_this(), RR_BOOST_PLACEHOLDERS(_1),
                                                         RR_BOOST_PLACEHOLDERS(_2), client3, url, key2),
                                             5000);
        ...
        {
            ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                               "FindServiceByType begin getting ServiceInfo2 from url "
                                                   << url << " failed: " << err2.what());
            try
            {
                node->AsyncDisconnectService(client, &Discovery_findservicebytype::rr_empty_handler);
...
void Discovery_findservicebytype::find2()
{
    ...
    BOOST_FOREACH (NodeDiscoveryInfo& ee, n1)
    {
        ...
        {
            ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                               "FindServiceByType error processing NodeDiscoveryInfo for NodeID "
                                                   << ee.NodeID.ToString() << ": " << exp.what());
        }
    }
...
void Discovery_updateserviceinfo::handle_error(const RR_SHARED_PTR<RobotRaconteurException>& err)
{
    ...
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo for remote node "
                                               << this->remote_nodeid.ToString()
                                               << " failed, retrying: " << err->what());
        ...
        RR_SHARED_PTR<Timer> t = n->CreateTimer(boost::posix_time::milliseconds(backoff),
                                                boost::bind(&Discovery_updateserviceinfo::backoff_timer_handler,
                                                            shared_from_this(), RR_BOOST_PLACEHOLDERS(_1)),
                                                true);
        t->Start();
    ...
    RobotRaconteurNode::TryPostToThreadPool(
        node, boost::bind(handler2, storage, RR_SHARED_PTR<std::vector<ServiceInfo2> >(), service_nonce, err), true);
}
                                                      ...
                                                      const RR_SHARED_PTR<RobotRaconteurException>& err)
{
    ...
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo getting ServiceInfo2 for remote node "
                                               << this->remote_nodeid.ToString() << " failed: " << err->what());
        handle_error(err);
        return;
    }
    ...
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo for remote node " << this->remote_nodeid.ToString()
                                                                                << " failed: Invalid return");
    ...
    if (ret1->Error != MessageErrorType_None)
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo getting ServiceInfo2 for remote node "
                                               << this->remote_nodeid.ToString() << " failed: " << ret1->Error);
    ...
    try
    {
        ...
        {
            ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                               "UpdateServiceInfo dropping response from remote node "
                                                   << this->remote_nodeid.ToString() << " to prevent overflow");
            handle_error(RR_MAKE_SHARED<ServiceException>("Return from remote node too large"));
            return;
        }
        RR_INTRUSIVE_PTR<RobotRaconteur::RRMap<int32_t, RobotRaconteurServiceIndex::ServiceInfo> > ret =
            RobotRaconteur::rr_cast<RobotRaconteur::RRMap<int32_t, RobotRaconteurServiceIndex::ServiceInfo> >(
                (n->UnpackMapType<int32_t, RobotRaconteurServiceIndex::ServiceInfo>(me->CastDataToNestedList())));

        if (ret)
        {
            BOOST_FOREACH (RR_INTRUSIVE_PTR<RobotRaconteurServiceIndex::ServiceInfo>& e,
                           *ret | boost::adaptors::map_values)
            {
                ServiceInfo2 o1;
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "UpdateServiceInfo getting ServiceInfo2 for remote node "
                                           << this->remote_nodeid.ToString() << " completed successfully");
                                                  ...
                                                  const RR_SHARED_PTR<RobotRaconteurException>& err)
{
    ...
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo connect for remote node "
                                               << this->remote_nodeid.ToString() << " failed: " << err->what());
        handle_error(err);
        return;
    }
    ...
    {
        ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo connected to remote node "
                                               << this->remote_nodeid.ToString() << ", begin getting ServiceInfo2");
        ...
        RR_INTRUSIVE_PTR<RobotRaconteur::MessageEntry> rr_req = RobotRaconteur::CreateMessageEntry(
            RobotRaconteur::MessageEntryType_FunctionCallReq, "GetLocalNodeServices");
        client3->AsyncProcessRequest(rr_req,
                                     boost::bind(&Discovery_updateserviceinfo::serviceinfo_handler, shared_from_this(),
...
void Discovery_updateserviceinfo::backoff_timer_handler(const TimerEvent& evt)
{
    ...
    catch (RobotRaconteurException& err)
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                           "UpdateServiceInfo connecting to remote node "
                                               << this->remote_nodeid.ToString() << " using candidate URLs "
                                               << boost::join(urls, ", ") << " failed: " << err.what());
        handle_error(RobotRaconteurExceptionUtil::DownCastException(err));
        return;
    }
    catch (std::exception& err)
    ...
    int32_t extra_backoff)
{
    ...
    RR_SHARED_PTR<Timer> t = n->CreateTimer(
        boost::posix_time::milliseconds(backoff),
...
std::vector<NodeDiscoveryInfo> Discovery::GetDetectedNodes()
{
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "GetDetectedNodes returning " << o.size() << " detected nodes");
...
bool Discovery::TryGetDetectedNodeCacheInfo(const RobotRaconteur::NodeID& nodeid, NodeInfo2& nodeinfo2)
{
    ...
    std::map<std::string, RR_SHARED_PTR<Discovery_nodestorage> >::iterator e1 =
        m_DiscoveredNodes.find(nodeid.ToString());
    if (e1 == m_DiscoveredNodes.end())
    {
        ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                           "TryGetDetectedNodeCacheInfo node " << nodeid.ToString() << " not found");
        return false;
    }

    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "TryGetDetectedNodeCacheInfo node " << nodeid.ToString() << " returning "
                                                                           << nodeinfo2.ConnectionURL.size()
                                                                           << " urls");
...
static std::string Discovery_log_NodeDiscoveryInfoURLs_url_field(const NodeDiscoveryInfoURL& url) { return url.URL; }
...
void Discovery::NodeDetected(const NodeDiscoveryInfo& info)
{
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "Node detected NodeID " << info.NodeID.ToString() << " NodeName "
                                                               << info.NodeName << " URLs "
                                                               << Discovery_log_NodeDiscoveryInfoURLs(info.URLs));

    if (info.ServiceStateNonce.size() > 32)
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(
            node, Discovery, -1, "Node detected NodeID " << info.NodeID.ToString() << " invalid ServiceStateNonce");
        return;
    }
    ...
    try
    {
        ...
        {
            ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(
                node, Discovery, -1, "Node detected NodeID " << info.NodeID.ToString() << " invalid NodeName");
            return;
        }
        ...
        if (e1 == m_DiscoveredNodes.end())
        {
            ...
            {
                ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                   "Detected node NodeID " << info.NodeID.ToString()
            ...
            for (std::vector<NodeDiscoveryInfoURL>::iterator e = info2->URLs.begin(); e != info2->URLs.end();)
            {
                ...
                if (valid)
                {
                    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(
                        node, Discovery, -1,
                        "Node detected NodeID " << info.NodeID.ToString() << " accepting candidate URL " << e->URL);
                    e++;
                }
                else
                {
                    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(
                        node, Discovery, -1,
                        "Node detected NodeID " << info.NodeID.ToString() << " rejecting candidate URL " << e->URL);
            ...
            if (info2->URLs.empty())
            {
                ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                                   "Node detected NodeID " << info.NodeID.ToString()
                                                                           << " did not find any valid candidate URLs");
                return;
            }
            ...
            {
                ROBOTRACONTEUR_LOG_TRACE_COMPONENT(
                    node, Discovery, -1, "Node detected NodeID " << info.NodeID.ToString() << " updating service info");
                update->AsyncUpdateServiceInfo(storage, info2->ServiceStateNonce,
        ...
        else
        {
            ...
            BOOST_FOREACH (const NodeDiscoveryInfoURL& e, info.URLs)
            {
                ...
                if (!found)
                {
                    ...
                    catch (std::exception& exp2)
                    {
                        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                           "Node detected NodeID " << info.NodeID.ToString()
                                                                                   << " provides invalid URL: " << e.URL
                                                                                   << ": " << exp2.what());
                        continue;
                    }
                    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                                       "Node detected NodeID " << info.NodeID.ToString()
                                                                               << " accepting candidate URL " << e.URL);
                    i->URLs.push_back(e);
                }
            }
            ...
            if (i->ServiceStateNonce != info.ServiceStateNonce && !info.ServiceStateNonce.empty())
            {
                ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                                   "Node detected NodeID " << info.NodeID.ToString()
                                                                           << " updating ServiceStateNonce");
                i->ServiceStateNonce = info.ServiceStateNonce;
            }

            if (!info.ServiceStateNonce.empty())
            {
                if (boost::range::find(e1->second->recent_service_nonce, info.ServiceStateNonce) !=
                    e1->second->recent_service_nonce.end())
                {
                    return;
                }
                else
    ...
    {
        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(
            node, Discovery, -1, "Node detected NodeID " << info.NodeID.ToString() << " failed: " << exp.what());
    }
}
                                     ...
                                     const RR_SHARED_PTR<RobotRaconteurException>& err)
{
    ...
    if (storage->last_update_nonce != storage->info->ServiceStateNonce)
    {
        ...
        {
            ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                               "ServiceStateNonce for " << storage->info->NodeID.ToString()
...
void Discovery::RetryUpdateServiceInfo(const RR_SHARED_PTR<Discovery_nodestorage>& storage)
{
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "UpdateServiceInfo retry requested for " << storage->info->NodeID.ToString());
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "UpdateServiceInfo retry requested for " << storage->info->NodeID.ToString()
                                                                                ...
                                                                                << " ms for retry " << retry_count);

    RR_SHARED_PTR<Discovery_updateserviceinfo> update = RR_MAKE_SHARED<Discovery_updateserviceinfo>(node);
    storage->updater = update;
    update->AsyncUpdateServiceInfo(storage, storage->info->ServiceStateNonce,
                                   boost::bind(&Discovery::EndUpdateServiceInfo, shared_from_this(),
                                               RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2),
                                               RR_BOOST_PLACEHOLDERS(_3), RR_BOOST_PLACEHOLDERS(_4)),
...
void Discovery::AsyncUpdateDetectedNodes(const std::vector<std::string>& schemes, boost::function<void()>& handler,
                                         int32_t timeout)
{
    RR_SHARED_PTR<RobotRaconteurNode> n = node.lock();
    if (!n)
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "UpdateDetectedNodes requested for schemes " << boost::join(schemes, ", "));
...
void Discovery::NodeAnnouncePacketReceived(boost::string_ref packet)
{
    ...
    try
    {
        ...
        if (packet.substr(0, magic.length()) == magic)
        {
            ...
            {
                ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                                   "Parsing discovery text packet failed: invalid URL");
                return;
            }
            ...
            {
                ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                   "Parsing discovery text packet URL failed: " << exp2.what());
                return;
            }
            ...
            {
                ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                   "Parsing discovery text packet attributes failed: " << exp2.what());
            }
...

    {
        ...
        BOOST_FOREACH (std::string& e, keys)
        {
            try
            {
                ...
                {
                    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                                       "Discovery lost node " << e1->second->info->NodeID.ToString());
                    ...
                    RobotRaconteurNode::TryPostToThreadPool(
                        node, boost::bind(&RobotRaconteurNode::FireNodeLost, n, e2->info));
                }
            }
            catch (std::exception& exp2)
            {
                ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                   "Error cleaning node " << e << ": " << exp2.what());
            }
        }
    }
}
                                       ...
                                       int32_t timeout)
{
    ...
    for (size_t i = 0; i < nodeids.size(); i++)
    {
        ...
        try
        {
            ...
            std::map<std::string, RR_SHARED_PTR<Discovery_nodestorage> >::iterator e1 =
                m_DiscoveredNodes.find(nodeids.at(i));
            if (e1 == m_DiscoveredNodes.end())
                continue;
            boost::mutex::scoped_lock lock2(e1->second->this_lock);
        ...
        {
            ROBOTRACONTEUR_LOG_TRACE_COMPONENT(
                node, Discovery, -1, "Error searching for candidate URLs for FindServiceByType: " << exp.what());
        }
    }
...
std::vector<ServiceInfo2> Discovery::FindServiceByType(boost::string_ref servicetype,
...
std::vector<NodeInfo2> Discovery::FindNodeByID(const RobotRaconteur::NodeID& id,
                                               const std::vector<std::string>& transportschemes)
{
    ROBOTRACONTEUR_ASSERT_MULTITHREADED(node);
    ...
    const boost::function<void(const RR_SHARED_PTR<std::vector<NodeInfo2> >&)>& handler)
{
                ...
                BOOST_FOREACH (const std::string& e, transportschemes)
                {
                    try
                    {
                        ...
                        if (u.scheme == e)
                        {
                            ...
                            if (u.port == -1)
                            {
                                short_url = u.scheme + "://" + u.host + u.path + "?nodeid=" +
                                            boost::replace_all_copy(
                                                boost::replace_all_copy(u.nodeid.ToString(), "{", ""), "}", "");
                            }
                            else
                            {
                                short_url = u.scheme + "://" + u.host + ":" + boost::lexical_cast<std::string>(u.port) +
                                            u.path + "?nodeid=" +
                                            boost::replace_all_copy(
                                                boost::replace_all_copy(u.nodeid.ToString(), "{", ""), "}", "");
                            }
                    ...
                    {
                        ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                           "AsyncFindNodeByID for remote node "
                                                               << id.ToString()
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "AsyncFindNodeByID for remote node "
                                           << id.ToString() << " completed successfully with " << ret->size()
                                           << " candidates");

    detail::InvokeHandler(node, handler, ret);
}

std::vector<NodeInfo2> Discovery::FindNodeByName(boost::string_ref name,
                                                 const std::vector<std::string>& transportschemes)
{
    ROBOTRACONTEUR_ASSERT_MULTITHREADED(node);
...
{
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "Begin AsyncFindNodeByName for remote node \"" << name << "\"");
    boost::function<void()> h = boost::bind(&Discovery::EndAsyncFindNodeByName, shared_from_this(), name.to_string(),
                                            transportschemes, handler);
    AsyncUpdateDetectedNodes(transportschemes, h, timeout);
}
    ...
    const boost::function<void(const RR_SHARED_PTR<std::vector<NodeInfo2> >&)>& handler)
{
                    ...
                    BOOST_FOREACH (const std::string& e, transportschemes)
                    {
                        try
                        {
                            ...
                            if (u.scheme == e)
                            {
                                ...
                                {
                                    short_url = u.scheme + "://" + u.host + ":" +
                                                boost::lexical_cast<std::string>(u.port) + u.path + "?nodeid=" +
                                                boost::replace_all_copy(
                                                    boost::replace_all_copy(u.nodeid.ToString(), "{", ""), "}", "");
                                }
                                n.ConnectionURL.push_back(short_url);
                        ...
                        catch (std::exception& exp2)
                        {
                            ROBOTRACONTEUR_LOG_DEBUG_COMPONENT(node, Discovery, -1,
                                                               "AsyncFindNodeByID for remote node \""
                                                                   << name << "\" failed processing candidate: "
                                                                   << exp2.what());
                        }
                    }
                }
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "AsyncFindNodeByID for remote node \""
                                           << name << "\" completed successfully with " << ret->size()
                                           << " candidates");
                            ...
                            const RR_SHARED_PTR<IServiceSubscription>& s)
{
    ...
    ROBOTRACONTEUR_LOG_TRACE_COMPONENT(node, Discovery, -1,
                                       "Begin DoSubscribe for service types " << boost::join(service_types, ", "));
...
void Discovery::DoUpdateAllDetectedServices(const RR_SHARED_PTR<IServiceSubscription>& s)
{
    ...
    BOOST_FOREACH (RR_SHARED_PTR<Discovery_nodestorage>& n, storage)
    {
        ...
        if (n->last_update_nonce != n->info->ServiceStateNonce || n->info->ServiceStateNonce.empty())
        {
            ...
            {
                ROBOTRACONTEUR_LOG_TRACE_COMPONENT(
                    node, Discovery, -1, "Subscription requesting update for node " << n->info->NodeID.ToString());
