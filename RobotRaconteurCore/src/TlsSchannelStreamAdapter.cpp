...
namespace detail
{
	...
	CredHandle TlsSchannelAsyncStreamAdapterContext::GetServerCredentials()
	{
		...
		if (server_credentials.dwLower==0 && server_credentials.dwUpper==0)
		{
			if (!activecertificate) throw InvalidOperationException("Server certificate not set");
			...
			Status_cred = AcquireCredentialsHandleA( NULL,                     
														UNISP_NAME_A,         
														SECPKG_CRED_INBOUND, 
														NULL,                 
														&SchannelCred,        
	...
	{		
		...
		::CertFreeCertificateChainEngine(hc);
		DWORD okerr = CERT_TRUST_HAS_NOT_SUPPORTED_CRITICAL_EXT | CERT_TRUST_INVALID_EXTENSION | CERT_TRUST_REVOCATION_STATUS_UNKNOWN | CERT_TRUST_IS_OFFLINE_REVOCATION;
		...
		BOOL cv_res=CertVerifyCertificateChainPolicy(CERT_CHAIN_POLICY_SSL,pChainContext,(PCERT_CHAIN_POLICY_PARA)&policy_para,&status);
		...
		if (valid)
		{
			if ((pChainContext->rgpChain[0]->TrustStatus.dwErrorStatus & (~(CERT_TRUST_HAS_NOT_SUPPORTED_CRITICAL_EXT | CERT_TRUST_INVALID_EXTENSION | CERT_TRUST_REVOCATION_STATUS_UNKNOWN | CERT_TRUST_IS_OFFLINE_REVOCATION | CERT_TRUST_IS_UNTRUSTED_ROOT))) != 0)
			{
				valid=false;
			}
	...
	void TlsSchannelAsyncStreamAdapter_ASIO_adapter::close()
	{
		next_layer_.close();
	}
	...
	void TlsSchannelAsyncStreamAdapter_close_cert_store(void* h)
	{
		CertCloseStore(h,0);
	}
	...
	void TlsSchannelAsyncStreamAdapter::async_handshake(boost::function<void (const boost::system::error_code&)> handler)
	{
		...
		if (direction==client)
		{
			...
			scRet = InitializeSecurityContextA( hCreds.get(),
                                                            NULL,
                                                            servername_c.get(),
                                                            dwSSPIFlags,
                                                            0,
			...
			if(OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL)
			{
				...
				handshaking=true;
				_async_write_some(send_buf,boost::bind(&TlsSchannelAsyncStreamAdapter::do_handshake1,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,boost::protect(handler)));
			}
			else
			{
		...
		if (direction==server)
		{
			...
			_async_read_some(recv_buf,boost::bind(&TlsSchannelAsyncStreamAdapter::do_handshake2,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,boost::protect(handler),true));
	...
	void TlsSchannelAsyncStreamAdapter::do_handshake1(const boost::system::error_code& error, size_t bytes_transferred, boost::function<void (const boost::system::error_code&)> handler)
	{
		...
		else
		{
			...
			_async_read_some(recv_b, boost::bind(&TlsSchannelAsyncStreamAdapter::do_handshake2,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,boost::protect(handler),true));
			return;
		}
	}

	void TlsSchannelAsyncStreamAdapter::do_handshake2(const boost::system::error_code& error, size_t bytes_transferred, boost::function<void (const boost::system::error_code&)> handler, bool doread)
	{
		boost::mutex::scoped_lock lock(stream_lock);
		do_handshake3(error,bytes_transferred,handler,doread);
	}

	void TlsSchannelAsyncStreamAdapter::do_handshake3(const boost::system::error_code& error, size_t bytes_transferred, boost::function<void (const boost::system::error_code&)> handler, bool doread)
	{
		...
		else
		{
			...
			if (!hContext)
			{
				CtxtHandle* phContext=new CtxtHandle();
				ZeroMemory(phContext,sizeof(CtxtHandle));
				scRet = AcceptSecurityContext(  hCreds.get(),
															NULL,
															pInBuffer,
															dwSSPIFlags,
															0,
		 ...
		 {
			 mutable_buffers recv_b;
			 recv_b.push_back(boost::asio::mutable_buffer(recv_buffer.get() + recv_buffer_end_pos, max_tls_record_size - recv_buffer_end_pos));
			 _async_read_some(recv_b,boost::bind(&TlsSchannelAsyncStreamAdapter::do_handshake2,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,boost::protect(handler),true));
		...
		{
			MoveMemory( recv_buffer.get(), recv_buffer.get() + (recv_buffer_end_pos-InBuffers[1].cbBuffer), InBuffers[1].cbBuffer );
		...
		if (send_buffer_end_pos!=0)
		{
			...
			if (scRet==SEC_E_OK)
			{
				_async_write_some(send_buf,boost::bind(&TlsSchannelAsyncStreamAdapter::do_handshake4,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,boost::protect(handler)));
			}
			else
			{
				_async_write_some(send_buf,boost::bind(&TlsSchannelAsyncStreamAdapter::do_handshake1,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,boost::protect(handler)));
	...
	void TlsSchannelAsyncStreamAdapter::do_handshake4(const boost::system::error_code& error, size_t bytes_transferred, boost::function<void (const boost::system::error_code&)> handler)
	{
		boost::mutex::scoped_lock lock(stream_lock);
		if (error)
		{
	...
	void TlsSchannelAsyncStreamAdapter::do_handshake6(const boost::system::error_code& error, boost::function<void (const boost::system::error_code&)> handler)
	{
		try
		{
			if (request_shutdown)
	...
	static void TlsSchannelAsyncStreamAdapter_async_write_some_buf_adaptor(RR_SHARED_PTR<TlsSchannelAsyncStreamAdapter> t, RR_SHARED_PTR<const_buffers> b, boost::function<void(const boost::system::error_code&, size_t)> handler)
	{
		t->async_write_some(*b, handler);
	}

	void TlsSchannelAsyncStreamAdapter::async_write_some(const_buffers& b, boost::function<void (const boost::system::error_code&, size_t)> handler)
	{
		boost::mutex::scoped_lock lock(stream_lock);
		...
		if (handshaking || shutingdown)
		{
			...
			async_write_op=boost::bind(&TlsSchannelAsyncStreamAdapter_async_write_some_buf_adaptor,shared_from_this(),b2,boost::protect(handler));
		...
		writing=true;
		_async_write_some(buf2,boost::bind(&TlsSchannelAsyncStreamAdapter::async_write_some1,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,Buffers[1].cbBuffer,boost::protect(handler)));
		
	}

	void TlsSchannelAsyncStreamAdapter::async_write_some1(const boost::system::error_code& error, size_t bytes_transferred, size_t len, boost::function<void (const boost::system::error_code&,size_t)> handler)
	{
		...
		const_buffers buf2;
		buf2.push_back(boost::asio::const_buffer(send_buffer.get() + send_buffer_transfer_pos, send_buffer_end_pos - send_buffer_transfer_pos));
		_async_write_some(buf2,boost::bind(&TlsSchannelAsyncStreamAdapter::async_write_some1,shared_from_this(),boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred,len,boost::protect(handler)));
	}

	void TlsSchannelAsyncStreamAdapter::async_write_some2(const boost::system::error_code& error, boost::function<void (const boost::system::error_code&, size_t)> handler)
	{
	...
	void TlsSchannelAsyncStreamAdapter::async_read_some(mutable_buffers& b, boost::function<void (const boost::system::error_code& , size_t )> handler) 
	{
		...
		boost::asio::mutable_buffer b3 = boost::asio::detail::buffer_sequence_adapter<boost::asio::mutable_buffer, mutable_buffers >::first(b);
		...
		else
		{
			...
			_async_read_some(recv_b,boost::bind(&TlsSchannelAsyncStreamAdapter::async_read_some1,shared_from_this(), b3, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, boost::protect(handler)));
			return;
		}
	}
	
	void TlsSchannelAsyncStreamAdapter::async_read_some1(boost::asio::mutable_buffer& b, const boost::system::error_code& error, size_t bytes_transferred, boost::function<void (const boost::system::error_code& , size_t )> handler) 
	{
		boost::mutex::scoped_lock lock(stream_lock);
		if (bytes_transferred==0)
		{
		...
		while (keepgoing)
		{
			...
			{
				mutable_buffers recv_b;
				recv_b.push_back(boost::asio::buffer(recv_buffer.get() + recv_buffer_end_pos, max_tls_record_size - recv_buffer_end_pos));
				_async_read_some(recv_b, boost::bind(&TlsSchannelAsyncStreamAdapter::async_read_some1,shared_from_this(), b, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, boost::protect(handler)));
				return;
			}
			...
			if ((scRet == SEC_E_OK || scRet == SEC_I_RENEGOTIATE)  && pExtraBuffer != NULL )
			{
				if (cbDataBuffer==0 && pExtraBuffer->cbBuffer!=0)
				{
					...
					//_async_read_some(boost::asio::buffer(recv_buffer+recv_buffer_end_pos,max_tls_record_size-recv_buffer_end_pos),boost::bind(&TlsSchannelAsyncStreamAdapter::async_read_some1,shared_from_this(), b, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, boost::protect(handler)));
	...
	void TlsSchannelAsyncStreamAdapter::async_read_some2(const boost::system::error_code& error, boost::asio::mutable_buffer& b, boost::function<void (const boost::system::error_code&, size_t)> handler) 
	{
		{
			//boost::mutex::scoped_lock lock(stream_lock);
	...
	void TlsSchannelAsyncStreamAdapter::do_shutdown1()
	{
		...
		if (direction==server)
		{
			Status = AcceptSecurityContext(hCreds.get(),
				hContext.get(),
				NULL,
				dwSSPIFlags,
				0,
		...
		{
		Status = InitializeSecurityContextA( hCreds.get(),
														hContext.get(),
														NULL,
														dwSSPIFlags,
														0,
	...
	boost::tuple<std::string,std::string> TlsSchannelAsyncStreamAdapter::GetTlsPublicKeys()
	{
		...
		c_l_64.resize(c_l_s);
		if(!CryptBinaryToStringA(c_l_pk.pbData,c_l_pk.cbData,CRYPT_STRING_BASE64 | CRYPT_STRING_NOCR,&c_l_64[0],&c_l_s))
		{
			::CertFreeCertificateContext(c_l);
			throw InternalErrorException("Internal error");
		}
		...
		std::string c_r_64;
		c_r_64.resize(c_r_s);
		if(!CryptBinaryToStringA(c_r_pk.pbData,c_r_pk.cbData,CRYPT_STRING_BASE64 | CRYPT_STRING_NOCR,&c_r_64[0],&c_r_s))
		{
			::CertFreeCertificateContext(c_r);
			throw InternalErrorException("Internal error");
		}
	...
	bool TlsSchannelAsyncStreamAdapter::get_mutual_auth()
	{
		return this->mutual_auth;
	}
	void TlsSchannelAsyncStreamAdapter::set_mutual_auth(bool mutual_auth)
	{
