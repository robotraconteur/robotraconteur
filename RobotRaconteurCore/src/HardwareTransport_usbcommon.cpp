...
namespace detail
{
	...
	UsbDevice_Claim_Lock::UsbDevice_Claim_Lock(RR_SHARED_PTR<UsbDevice_Claim> parent)
	{
		this->parent = parent;
	}
	...
	RR_SHARED_PTR<HardwareTransport> UsbDeviceManager::GetParent()
	{
		...
		if (!t) throw InvalidOperationException("Hardware transport has been released");
		return t;
	}
	...
	void UsbDeviceManager::UpdateDevices(boost::function<void()> handler)
	{
		...
		if (!RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDeviceManager::UpdateDevices1, shared_from_this(), boost::protect(handler))))
		{
			RobotRaconteurNode::TryPostToThreadPool(node, handler, true);
		}
	}
	...
	void UsbDeviceManager::UpdateDevices2(UsbDeviceStatus status, RR_SHARED_PTR<UsbDevice> dev, RR_SHARED_PTR< std::list < RR_SHARED_PTR<UsbDevice> > > l, boost::function<void()> handler)
	{
		boost::mutex::scoped_lock lock(this_lock);
	...
	void UsbDeviceManager::AsyncCreateTransportConnection1(const ParseConnectionURLResult& url_res, uint32_t endpoint, const std::string& noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler)
	{
		...
		handler(RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Could not connect to device or device is busy"));
	...
	UsbDevice_Initialize::UsbDevice_Initialize(RR_SHARED_PTR<UsbDevice> parent, const UsbDeviceManager_detected_device& detected_device)
	{
		this->parent = parent;
		this->node = parent->GetNode();
	...
	RR_SHARED_PTR<UsbDevice> UsbDevice_Initialize::GetParent()
	{
		...
		if (!d) throw InvalidOperationException("Parent has been released");
		return d;
	}
	...
	void UsbDevice_Initialize::InitializeDevice(boost::function< void(UsbDeviceStatus) > handler)
	{
		...
		status = Initializing;
		uint32_t attempt = 0;
		if (!RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDevice_Initialize::InitializeDevice1, shared_from_this(), 0, boost::protect(handler), RR_SHARED_PTR<boost::asio::deadline_timer>())))
		{
			RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(handler, Shutdown), true);
		}
	}

	void UsbDevice_Initialize::InitializeDevice1(uint32_t attempt, boost::function< void(UsbDeviceStatus) > handler, RR_SHARED_PTR<boost::asio::deadline_timer> timer)
	{
		boost::mutex::scoped_lock lock(this_lock);
	...
	void UsbDevice_Initialize::InitializeDevice_err(boost::function< void(UsbDeviceStatus) >& handler, UsbDeviceStatus status1)
	{
		RR_SHARED_PTR<ThreadPool> p = GetNode()->GetThreadPool();
	...
	void UsbDevice_Initialize::ReadRRDeviceString(uint8_t interface_number, uint8_t property_index, boost::function< void(const boost::system::error_code&, const std::string&) > handler, RR_SHARED_PTR<void> dev_h)
	{		
	...
	void UsbDevice_Initialize::ReadRRDeviceString2(const boost::system::error_code& ec, size_t bytes_transferred, uint8_t interface_number, uint8_t property_index, boost::shared_array<uint8_t> buf, boost::function< void(const boost::system::error_code&, const std::string&) > handler, RR_SHARED_PTR<void> dev_h)
	{
		if (ec)
		{
			handler(ec, "");
	...
	UsbDevice_Claim::UsbDevice_Claim(RR_SHARED_PTR<UsbDevice> parent, const UsbDeviceManager_detected_device& detected_device)
	{
		this->parent = parent;
		this->node = parent->GetNode();
	...
	void UsbDevice_Claim::AsyncCreateTransportConnection1(const ParseConnectionURLResult& url_res, uint32_t endpoint, const std::string& noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler, uint32_t attempt)
	{
		boost::mutex::scoped_lock lock(this_lock);
		...
		case Closed:		
		{
			RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("USB Device Released. Try again.")), true);
			stream_connection_in_progress--;
			break;
		}
		default:
			RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("USB Device Error.")), true);
			stream_connection_in_progress--;
			break;
		}
	}

	void UsbDevice_Claim::AsyncCreateTransportConnection2(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf, const ParseConnectionURLResult& url_res, uint32_t endpoint, const std::string& noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler)
	{
	...
	void UsbDevice_Claim::AsyncCreateTransportConnection4(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf, const ParseConnectionURLResult& url_res, uint32_t endpoint, const std::string& noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler)
	{
		if (ec)
		{
			{
...

		{
			...
			{
				if (!RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDevice_Claim::AsyncCreateTransportConnection1, shared_from_this(), req.url_res, req.endpoint, req.noden, boost::protect(req.handler), 0)))
				{
					RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(req.handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Node shutdown")), true);
				}
			}
	...
	void UsbDevice_Claim::AsyncCreateTransportConnection5(const ParseConnectionURLResult& url_res, uint32_t endpoint, const std::string& noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler)
	{		
		boost::shared_array<uint8_t> buf(new uint8_t[4]);
	...
	void UsbDevice_Claim::AsyncCreateTransportConnection6(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf, const ParseConnectionURLResult& url_res, uint32_t endpoint, const std::string& noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler)
	{
		try
		{
			...
			if (!usb_dev) throw InvalidOperationException("USBDevice has been released");
			RR_SHARED_PTR<HardwareTransport> hw=usb_dev->GetParent()->GetParent();

			RR_SHARED_PTR<UsbDeviceTransportConnection> t = RR_MAKE_SHARED<UsbDeviceTransportConnection>(GetParent()->GetParent()->GetParent(), endpoint, shared_from_this(), id);
		...
		{
			handler(RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Error creating USB transport connection"));
		}
	}

	//Call with this_lock locked
	void UsbDevice_Claim::AsyncCreateTransportConnection_err(boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)>& handler, UsbDeviceStatus status)
	{
		RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("USB Device Error")), true);

		BOOST_FOREACH(UsbDeviceClaim_create_request& req, create_requests)
		{
			RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(req.handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("USB Device Error")), true);
		}
	...
	void UsbDevice_Claim::CleanupConnections()
	{
		...
		if (stream_connection_in_progress == 0 && transport_connections.empty() && claim_locks.empty())
		{
			...
			try
			{
				RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDevice_Claim::Close, shared_from_this()), true);
			}
			catch (std::exception&) {}
		}
	}
	...
	void UsbDevice_Claim::ConnectionClosed(RR_SHARED_PTR<UsbDeviceTransportConnection> connection)
	{
		try
		{
			int32_t id;
			{
				...
				std::map<int32_t, RR_SHARED_PTR<UsbDeviceTransportConnection> >::iterator e=transport_connections.find(id);
	...
	void UsbDevice_Claim::ConnectionClosed1(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf)
	{
		boost::mutex::scoped_lock lock(this_lock);
		CleanupConnections();
	}
	...
	void UsbDevice_Claim::DoWrite()
	{
		while (write_buf.size() > 0)
		{
			...
			if (transport_write_idle.size() == 0 && transport_write.size() == 0) return;

			if (transport_write.size() == 0)
			{
				for (std::list<RR_WEAK_PTR<UsbDeviceTransportConnection> >::iterator e = transport_write_idle.begin(); e != transport_write_idle.end();)				
				{
					RR_SHARED_PTR<UsbDeviceTransportConnection> c = e->lock();
					if (!c)
					{
			...
			for(std::list<RR_WEAK_PTR<UsbDeviceTransportConnection> >::iterator e=transport_write.begin(); e!=transport_write.end() && write_buf.size() > 0;)
			{
				RR_SHARED_PTR<UsbDeviceTransportConnection> c = e->lock();
				if (!c)
				{
	...
	void UsbDevice_Claim::EndRead(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf, uint64_t c)
	{
		if (ec)
		{
			DeviceError(ec);
			...
			while (!read_out_of_order.empty() && status==Claimed)
			{
				...
				for(e_type e2=read_out_of_order.begin(); e2!=read_out_of_order.end(); )
				{
					if (e2->get<2>() == read_last_complete + 1)
					{
						...
						read_last_complete++;
						found = true;						
						std::list<boost::shared_array<uint8_t> >::iterator e3 = boost::range::find(read_buf_use, e2->get<1>());
	...
	{	
		...
		{	
			RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDevice_Claim::DeviceError,shared_from_this(), boost::asio::error::fault), true);
			return;
		}
			...
			if (boost::asio::buffer_size(b1) >= 2)
			{
				...
				case RR_USB_NOTIFICATION_SOCKET_CLOSED:
				{
					...
					if (e != transport_connections.end())
					{
						...
						transport_write_remove(transport_write_idle, e->second->stream_id);
						transport_connections.erase(e);						
						RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDeviceTransportConnection::Close, t), true);
					}
					break;
				}
				...
				case RR_USB_NOTIFICATION_RESUME_REQUEST:
				{
					...
					if (e != transport_connections.end())
					{
						RR_SHARED_PTR<UsbDeviceTransportConnection>& t = e->second;
						t->send_paused = false;
						RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(&UsbDevice_Claim::TransportCanWrite, shared_from_this()));
					}
					break;
				}
				default:
	...
	void UsbDevice_Claim::EndWrite(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf)
	{
		if (ec)
		{
			DeviceError(ec);
	...
	void UsbDevice_Claim::DeviceError(const boost::system::error_code& ec)
	{
		Close();
	}
	...
	void UsbDevice_Claim::Closed1(const boost::system::error_code& ec, size_t bytes_transferred, boost::shared_array<uint8_t> buf)
	{
	...
	void UsbDevice_Claim::Closed2()
	{
		...
		if (!create_requests.empty())
		{
			...
			BOOST_FOREACH(UsbDeviceClaim_create_request& req, create_requests)
			{
				RobotRaconteurNode::TryPostToThreadPool(node,boost::bind(req.handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("USB Device Released. Try again.")), true);
			}
	...
	RR_SHARED_PTR<UsbDeviceManager> UsbDevice::GetParent()
	{
		...
		if (!m) throw InvalidOperationException("UsbDeviceManager has been released");
		return m;
	}
	...
	void UsbDevice::AsyncCreateTransportConnection(const ParseConnectionURLResult& url_res, uint32_t endpoint, boost::string_ref noden, boost::function<void(RR_SHARED_PTR<ITransportConnection>, RR_SHARED_PTR<RobotRaconteurException>)> handler)
	{
		...
		{
			RobotRaconteurNode::TryPostToThreadPool(node, boost::bind(handler, RR_SHARED_PTR<ITransportConnection>(), RR_MAKE_SHARED<ConnectionException>("Invalid USB device status")), true);
			return;
		}
	...
	UsbDeviceTransportConnection::UsbDeviceTransportConnection(RR_SHARED_PTR<HardwareTransport> parent, uint32_t local_endpoint, RR_SHARED_PTR<UsbDevice_Claim> device, int32_t stream_id) 
	...
	{
		if (!connected.load()) throw ConnectionException("Connection lost");
	...
	void UsbDeviceTransportConnection::async_read_some(mutable_buffers& b, boost::function<void(const boost::system::error_code& error, size_t bytes_transferred)>& handler)
	{
		if (!connected.load()) throw ConnectionException("Connection lost");
	...
	size_t UsbDeviceTransportConnection::available()
	{
		return 0;
	}
	...
	void UsbDeviceTransportConnection::Close()
	{
		...
		try
		{
			...
			if (p) p->erase_transport(RR_STATIC_POINTER_CAST<UsbDeviceTransportConnection>(shared_from_this()));
		}
		catch (std::exception&) {}
		...
		try
		{
			...
			{
				boost::mutex::scoped_lock lock(do_read_lock);
				typedef boost::tuple<mutable_buffers, boost::function<void(const boost::system::error_code&, size_t)> > e_type;
			...
			{
				boost::mutex::scoped_lock lock(do_write_lock);
				typedef boost::tuple<const_buffers, boost::function<void(const boost::system::error_code&, size_t)> > e_type;
	...
	size_t UsbDeviceTransportConnection::CanDoRead()
	{
	...
	void UsbDeviceTransportConnection::DoRead(boost::asio::const_buffer& buf)
	{
		...
		{
			boost::tuple<mutable_buffers, boost::function<void(const boost::system::error_code&, size_t)> > op = read_ops.front();
			read_ops.pop_front();
	...
	size_t UsbDeviceTransportConnection::DoWrite(boost::asio::mutable_buffer& buf)
	{
		...
		boost::tuple<const_buffers, boost::function<void(const boost::system::error_code&, size_t)> > op = write_ops.front();
		write_ops.pop_front();
