...
namespace RobotRaconteur
{
	...
	{
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node,Client,endpoint,service_path,name,"Requesting generator abort with id " << id);
	...
	{
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node,Client,endpoint,service_path,name,"Requesting generator close with id " << id);
		RR_INTRUSIVE_PTR<MessageEntry> m = CreateMessageEntry(MessageEntryType_GeneratorNextReq, GetMemberName());
	...
	{
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node,Client,endpoint,service_path,name,"Calling generator next with id " << id);
		RR_INTRUSIVE_PTR<MessageEntry> m = CreateMessageEntry(MessageEntryType_GeneratorNextReq, GetMemberName());		
		m->AddElement("index", ScalarToRRArray(id));
		if (v)
		{
	...
	GeneratorServerBase::GeneratorServerBase(boost::string_ref name, int32_t index, RR_SHARED_PTR<ServiceSkel> skel, RR_SHARED_PTR<ServerEndpoint> ep)
	{
		...
		ROBOTRACONTEUR_LOG_TRACE_COMPONENT_PATH(node,Service,ep->GetLocalEndpoint(),service_path,name,"Created generator skel with id " << index);
	}
