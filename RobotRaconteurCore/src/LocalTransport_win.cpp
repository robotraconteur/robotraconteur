	...
	namespace LocalTransportUtil
	{
		...
		bool IsPipeUser(HANDLE pipe, std::string username)
		{
			...
			try
			{
				...
				if (pipe_username != username) valid = false;
		...
		std::wstring GetSIDStringForProcessId(DWORD processid)
		{
			...
			else
			{
				...
				if (hProc == INVALID_HANDLE_VALUE) throw SystemResourceException("Internal error");
		...
		std::string GetUserNameForProcessId(DWORD processid)
		{
			...
			if (!::LookupAccountSidW(NULL, ((PTOKEN_USER)pTokenUser.get())->User.Sid, username1, &cchName, lpReferencedDomainName, &cchReferencedDomainName, &peUse))
			{
		...
		{
			if (h == INVALID_HANDLE_VALUE) return RR_SHARED_PTR<std::ifstream>();
		...
		{
			if (h == INVALID_HANDLE_VALUE) return RR_SHARED_PTR<std::ofstream>();
		...
		{
			if (h == INVALID_HANDLE_VALUE) return RR_SHARED_PTR<std::fstream>();
...
namespace detail
{
	...
	{
		RR_SHARED_PTR<void> o(FindFirstChangeNotificationW(p.wstring().c_str(), TRUE, FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_SECURITY), &FindCloseChangeNotification);
		if (o.get() == INVALID_HANDLE_VALUE) o.reset();
		return o;
	}


	WinLocalTransportDiscovery::WinLocalTransportDiscovery(RR_SHARED_PTR<RobotRaconteurNode> node) : LocalTransportDiscovery(node)
	{
	...
	void WinLocalTransportDiscovery::Init()
	{
		...
		if (shutdown_evt.get() == INVALID_HANDLE_VALUE) throw SystemResourceException("Internal error");
		
		boost::thread(boost::bind(&WinLocalTransportDiscovery::run, shared_from_this()));
	}

	void WinLocalTransportDiscovery::Shutdown()
	{
		SetEvent(shutdown_evt.get());
	}
			...
			if (!public_evt)
			{
				try
				{
					...
					if (public_path)
					{
						...
						ROBOTRACONTEUR_LOG_TRACE_SOURCE(node, Transport, -1, "LocalTransport discovery watching public path " << *private_path)
					}					
				}
				catch (std::exception&) {}
			}
