...
namespace RobotRaconteurGen
{
	std::string CSharpServiceLangGen::fix_name(const std::string& name)
	{
		...
		const char* res_str[]={"abstract","as","async","await","base","bool","break","byte","case","catch","char","checked","class",
	...
	std::string CSharpServiceLangGen::fix_qualified_name(const std::string& name)
	{
	...
	CSharpServiceLangGen::convert_type_result CSharpServiceLangGen::convert_type(const TypeDefinition& tdef)
	{
		...
		if (tdef.ArrayType == DataTypes_ArrayTypes_multidimarray)
		{
			...
			{			
				...
				{
					o.cs_type = "AStructureMultiDimArray";
					o.cs_arr_type = "";
					break;
				}
				default:
	...
	std::string CSharpServiceLangGen::str_pack_parameters(const std::vector<RR_SHARED_PTR<TypeDefinition> >& l, bool inclass)
	{
	...
	std::string CSharpServiceLangGen::str_pack_delegate(const std::vector<RR_SHARED_PTR<TypeDefinition> >& l, boost::shared_ptr<TypeDefinition> rettype)
	{
		...
		case DataTypes_ContainerTypes_none:
		{
			...
			else if (t->Type == DataTypes_namedtype_t)
			{
				...
				case DataTypes_astructure_t:
					...
					case DataTypes_ArrayTypes_none:
					{
						return "MessageElementUtil.PackAStructureToArray<" + fix_qualified_name(t->TypeString) + ">(\"" + elementname + "\",ref " + varname + ")";
						break;
					}
					case DataTypes_ArrayTypes_array:
					{
						return "MessageElementUtil.PackAStructureArray<" + fix_qualified_name(t->TypeString) + ">(\"" + elementname + "\"," + CSharpServiceLangGen_VerifyArrayLength(*t, varname) + ")";
						break;
					}
					case DataTypes_ArrayTypes_multidimarray:
					{
						return "MessageElementUtil.PackAStructureMultiDimArray<" + fix_qualified_name(t->TypeString) + ">(\"" + elementname + "\"," + CSharpServiceLangGen_VerifyArrayLength(*t, varname) + ")";
						break;
					}
					default:
						throw DataTypeException("Invalid array type");
	...
	std::string CSharpServiceLangGen::str_unpack_message_element(const std::string &varname, const RR_SHARED_PTR<TypeDefinition> &t, const std::string &packer)
	{
		...
		else if (t->Type == DataTypes_namedtype_t)
		{
			...
			case DataTypes_astructure_t:
				...
				case DataTypes_ArrayTypes_none:
				{
					structunpackstring = "MessageElementUtil.UnpackAStructureFromArray<" + fix_qualified_name(t->TypeString) + ">(" + varname + ")";
					break;
				}
				case DataTypes_ArrayTypes_array:
				{
					structunpackstring = CSharpServiceLangGen_VerifyArrayLength(*t, "MessageElementUtil.UnpackAStructureArray<" + fix_qualified_name(t->TypeString) + ">(" + varname + ")");
					break;
				}
				case DataTypes_ArrayTypes_multidimarray:
				{
					structunpackstring = CSharpServiceLangGen_VerifyArrayLength(*t, "MessageElementUtil.UnpackAStructureMultiDimArray<" + fix_qualified_name(t->TypeString) + ">(" + varname + ")");
					break;
				}
				default:
					throw DataTypeException("Invalid array type");
	...
	void CSharpServiceLangGen::GenerateCStructure(RR_SHARED_PTR<ServiceEntryDefinition> e, ostream* w)
	{
		ostream& w2 = *w;

		if (e->EntryType == DataTypes_astructure_t)
		{
			boost::tuple<DataTypes, size_t> t4 = GetAStructureElementTypeAndCount(e);
			TypeDefinition t5;
			t5.Type = t4.get<0>();
			convert_type_result t6 = convert_type(t5);
			w2 << "[AStructureElementTypeAndCount(typeof(" << t6.cs_type << "), " << t4.get<1>() << ")]" << endl;
		}
	...
	void CSharpServiceLangGen::GenerateAStructureExtensions(RR_SHARED_PTR<ServiceEntryDefinition> e, ostream* w)
	{
		ostream& w2 = *w;

		boost::tuple<DataTypes, size_t> t1 = GetAStructureElementTypeAndCount(e);
		TypeDefinition t2;
		t2.Type = t1.get<0>();
	...
	void CSharpServiceLangGen::GenerateInterface(ServiceEntryDefinition* e, ostream* w)
	{
		...
		if (e->Implements.size() >0) implements=" : " + implements;
	...
	void CSharpServiceLangGen::GenerateInterfaceFile(ServiceDefinition* d, ostream* w)
	{
		ostream& w2=*w;
	...
	void CSharpServiceLangGen::GenerateStubSkelFile(ServiceDefinition* d, std::string defstring, ostream* w)
	{
		ostream& w2=*w;
		...
		for (std::vector<RR_SHARED_PTR<ServiceEntryDefinition> >::const_iterator e = d->AStructures.begin(); e != d->AStructures.end(); ++e)
		{
			GenerateAStructureExtensions(*e, w);
		}
		for (std::vector<RR_SHARED_PTR<ServiceEntryDefinition> >::const_iterator e = d->Objects.begin(); e != d->Objects.end(); ++e)
		{
	...
	{ 
		...
		w2 << "    public override IAStructureStub FindAStructureStub(string objecttype)" << endl << "    {" << endl;
		//w2 << "    string objshort=RemovePath(objecttype);" << endl;
		...
		w2 << "    string objecttype=innerstub.RR_objecttype.GetServiceDefinition().Name + \".\" + innerstub.RR_objecttype.Name;";
		...
		w2 << "    string objtype = RobotRaconteurNode.GetTypeString(ServiceSkelUtil.FindParentInterface(obj.GetType()));" << endl;
		w2 << "    string objshort;" << endl;
		...
		w2 << "    public override RobotRaconteurException DownCastException(RobotRaconteurException rr_exp)" << "{" << endl;
	...
	void CSharpServiceLangGen::GenerateStructureStub(ServiceEntryDefinition* e, ostream * w)
	{
		...
		w2 << "    T st; try {st=(T)((object)s);} catch (InvalidCastException) {throw new DataTypeMismatchException(\"Wrong structuretype\");}" << endl;
		w2 << "    return st;" << endl;
		w2 << "    }" << endl;
		w2 << "    }" << endl;
	...
	void CSharpServiceLangGen::GenerateAStructureStub(RR_SHARED_PTR<ServiceEntryDefinition> e, ostream * w)
	{
		ostream& w2 = *w;
		...
		w2 << "public class " + fix_name(e->Name) + "_stub : AStructureStub<" << fix_name(e->Name) << "," << t6.cs_type << "> {" << endl;
		w2 << "    public override " << t6.cs_type <<"[] GetNumericArrayFromAStructure(ref " << fix_name(e->Name) << " s) {" << endl;
		w2 << "    return s.GetNumericArray();" << endl;
		w2 << "    }" << endl;
		w2 << "    public override " << fix_name(e->Name) << " GetAStructureFromNumericArray(" << t6.cs_type << "[] m) {" << endl;
		w2 << "    if (m.Length != " << t4.get<1>() << ") throw new DataTypeException(\"Invalid astructure array\");" << endl;
		w2 << "    var s = new " << fix_name(e->Name) << "();" << endl;
		w2 << "    var a = new ArraySegment<" << t6.cs_type << ">(m);" << endl;
		w2 << "    s.AssignFromNumericArray(ref a);" << endl;
		w2 << "    return s;" << endl;
		w2 << "    }" << endl;
		w2 << "    public override " << t6.cs_type << "[] GetNumericArrayFromAStructureArray(" << fix_name(e->Name) << "[] s) {" << endl;
		w2 << "    return s.GetNumericArray();" << endl;
		w2 << "    }" << endl;
		w2 << "    public override " << fix_name(e->Name) << "[] GetAStructureArrayFromNumericArray(" << t6.cs_type << "[] m) {" << endl;
	...
	void CSharpServiceLangGen::GenerateStub(ServiceEntryDefinition* e, ostream * w)
	{
		...
		w2 << "public class " + fix_name(e->Name) + "_stub : ServiceStub , " + fix_name(e->Name) + ", async_" + fix_name(e->Name) + "{" << endl;
		...
		MEMBER_ITER2(PipeDefinition)
		convert_type_result t=convert_type(*m->Type);
		w2 << "    rr_" + fix_name(m->Name) + "=new Pipe<" +  t.cs_type + t.cs_arr_type + ">(innerstub.GetPipe(\"" + m->Name +"\"));" << endl;
		MEMBER_ITER_END()

		MEMBER_ITER2(WireDefinition)
		convert_type_result t=convert_type(*m->Type);
		w2 << "    rr_" + fix_name(m->Name) + "=new Wire<" +  t.cs_type + t.cs_arr_type + ">(innerstub.GetWire(\"" + m->Name +"\"));" << endl;
		MEMBER_ITER_END()
		...
		{
		case DataTypes_ArrayTypes_array:
			w2 << "    rr_" << fix_name(m->Name) << "=new " << c << "ArrayMemoryClient<" + t.cs_type + ">(innerstub.Get" << c << "ArrayMemory(\"" + m->Name + "\"));" << endl;
			break;
		case DataTypes_ArrayTypes_multidimarray:			
			w2 << "    rr_" << fix_name(m->Name) << "=new " << c << "MultiDimArrayMemoryClient<" + t.cs_type + ">(innerstub.Get" << c << "MultiDimArrayMemory(\"" + m->Name + "\"));" << endl;
			break;
		default:
			throw DataTypeException("Invalid memory definition");
		}
		...
		if (!m->IsGenerator())
		{
			...
			w2 << "    using(MessageElement rr_me=rr_innerstub.FunctionCall(\"" + m->Name + "\",rr_param))" << endl << "    {" << endl;
			if (m->ReturnType->Type != DataTypes_void_t)
			{
		...
		else
		{
			...
			w2 << "    return new " << t.generator_csharp_base_type << "Client<" << t.generator_csharp_template_params << ">(generator_client);" << endl;
		...
		else
		{
			...
			{			
				w2 << "    public " + objtype + " get_" + fix_name(m->Name) + "(" + indtype + " ind) {" << endl;
				w2 << "    return (" + objtype + ")FindObjRefTyped(\"" + fix_name(m->Name) + "\",ind.ToString(),\"" << objecttype2 << "\");" << endl;
				w2 << "    }" << endl;
			}
			else
			{
		...
		MEMBER_ITER2(CallbackDefinition)
		w2 << "    public Callback<" + str_pack_delegate(m->Parameters,m->ReturnType) + "> " + fix_name(m->Name) + " {" << endl;
		w2 << "    get { return rr_" + fix_name(m->Name) +  ";  }" << endl;
		w2 << "    set { throw new InvalidOperationException();}" << endl;
		w2 << "    }" << endl;
		MEMBER_ITER_END()
		...
		w2 << "    public override MessageElement CallbackCall(string rr_membername, vectorptr_messageelement rr_m) {" << endl;
		w2 << "    switch (rr_membername) {" << endl;
		MEMBER_ITER2(CallbackDefinition)
		string params=str_pack_parameters(m->Parameters,false);
		w2 << "    case \"" + m->Name + "\":" << endl << "    {" << endl;
	...
	void CSharpServiceLangGen::GenerateSkel(ServiceEntryDefinition* e, ostream * w)
	{
		...
		if (!m->IsGenerator())
		{
			...
			{
				convert_type_result t = convert_type(*m->ReturnType);
				w2 << "    " + t.cs_type + t.cs_arr_type + " rr_ret=this.obj." + fix_name(m->Name) + "(" + params + ");" << endl;
		...
		{		
			...
			w2 << "    int generator_index = innerskel.RegisterGeneratorServer(\"" << m->Name << "\", new Wrapped" << t4.generator_csharp_base_type << "ServerDirectorNET<" << t4.generator_csharp_template_params << ">(rr_ret));" << endl;
		...
		string params=str_pack_parameters(m->Parameters,true);
		w2 << "    case \"" + m->Name + "\": {" << endl;
		w2 << "    return new " + str_pack_delegate(m->Parameters,m->ReturnType)  + "( delegate(" + str_pack_parameters(m->Parameters,true) + ") {" << endl;;
		w2 << "    using(vectorptr_messageelement rr_param=new vectorptr_messageelement())" << endl << "    {" << endl;
		for (vector<RR_SHARED_PTR<TypeDefinition> >::const_iterator p=m->Parameters.begin(); p!=m->Parameters.end(); ++p)
		{
			w2 << "    MessageElementUtil.AddMessageElementDispose(rr_param," << str_pack_message_element((*p)->Name, fix_name((*p)->Name), *p) << ");" << endl;
		}
		w2 << "    using(MessageElement rr_me=this.innerskel.WrappedCallbackCall(\"" + m->Name + "\",rr_endpoint,rr_param))" << endl << "    {" << endl;
		if (m->ReturnType->Type != DataTypes_void_t)
		{
			w2 << "    return " << str_unpack_message_element("rr_me", m->ReturnType) + ";" << endl;
		}
		...
		MEMBER_ITER2(PipeDefinition)
		convert_type_result t=convert_type(*m->Type);
		w2 << "    obj." + fix_name(m->Name) + "=new Pipe<" + t.cs_type + t.cs_arr_type + ">(innerskel.GetPipe(\"" + m->Name+ "\"));" << endl;
		MEMBER_ITER_END()
		w2 << "    }" << endl;
		...
		w2 << "    obj." + fix_name(m->Name) + "=new CallbackServer<" + str_pack_delegate(m->Parameters,m->ReturnType)+ ">(\"" + m->Name+ "\",this);" << endl;
		MEMBER_ITER_END()
		w2 << "    }" << endl;
		...
		MEMBER_ITER2(WireDefinition)
		convert_type_result t=convert_type(*m->Type);
		w2 << "    obj." + fix_name(m->Name) + "=new Wire<" + t.cs_type + t.cs_arr_type + ">(innerskel.GetWire(\"" + m->Name+ "\"));" << endl;
		MEMBER_ITER_END()
		w2 << "    }" << endl;
		...
		{
			w2 << "    case \"" + m->Name + "\": {" << endl;
			w2 << "    WrappedArrayMemoryDirectorNET<" +  t.cs_type + "> dir=new  WrappedArrayMemoryDirectorNET<" +  t.cs_type + ">(obj." + fix_name(m->Name) + ");" << endl;
		...
		{
			w2 << "    case \"" + m->Name + "\": {" << endl;
			w2 << "    WrappedMultiDimArrayMemoryDirectorNET<" +  t.cs_type + "> dir=new  WrappedMultiDimArrayMemoryDirectorNET<" +  t.cs_type + ">(obj." + fix_name(m->Name) + ");" << endl;
			//w2 << "    int id=RRObjectHeap.AddObject(dir); " << endl;
			//w2 << "    dir.memoryid=id;" << endl;
			//w2 << "    dir.Disown();" << endl;
			w2 << "    return dir;" << endl;
	...
	std::string CSharpServiceLangGen::convert_constant(ConstantDefinition* c, std::vector<RR_SHARED_PTR<ConstantDefinition> >& c2, ServiceDefinition* def)
	{
		...
		if (t->Type==DataTypes_string_t)
		{
			...
			boost::regex_replace(oi, str_value.begin(), str_value.end(), r_replace, CSharpServiceLangGen_EscapeString_Formatter,
	...
	void CSharpServiceLangGen::GenerateConstants(ServiceDefinition*d, ostream* w)
	{
		...
		w2 << "public static class " << boost::replace_all_copy(fix_name(d->Name),".","__") << "Constants " << endl << "{" << endl;
	...
	std::string CSharpServiceLangGen::GetDefaultInitializedValue(const TypeDefinition& tdef)
	{
		...
		if (tdef.ContainerType == DataTypes_ContainerTypes_none)
		{
			...
			if (tdef.Type == DataTypes_namedtype_t)
			{
				...
				if (tdef2->ResolveNamedType()->RRDataType() == DataTypes_astructure_t)
				{
					switch (tdef.ArrayType)
					{
					case DataTypes_ArrayTypes_none:
					...
					case DataTypes_ArrayTypes_multidimarray:
					{
						...
						if (tdef.ArrayVarLength)
						{
							return "new AStructureMultiDimArray(new int[] {1,0}, new " + t.cs_type +  "[0])";
						}
						else
						{
							int32_t n_elems = boost::accumulate(tdef.ArrayLength, 1, std::multiplies<int32_t>());
							return "new AStructureMultiDimArray(new int[] {" + boost::join(tdef.ArrayLength | boost::adaptors::transformed(boost::lexical_cast<std::string, int32_t>), ",") + "}, new " + t.cs_type + "[" + boost::lexical_cast<std::string>(n_elems) + "])";
						}
					}
					default:
						throw InvalidArgumentException("Invalid array type");
