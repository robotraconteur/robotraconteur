...
if not vcpkg_dir.is_dir():
    subprocess.check_call(["git", "clone", "--depth=1", "https://github.com/microsoft/vcpkg.git"], cwd=build_dir)
...
shutil.copy(ports_in_dir / "robotraconteur/portfile.cmake", ports_rr_dir / "portfile.cmake")
...
for triplet in ios_triplets:
    ...
    subprocess.check_call([f"libtool -static -o {combined_lib_path} *.a"],shell=True,cwd=vcpkg_dir / f"installed/{triplet}/debug/lib")
...
subprocess.check_call(f"lipo {lib_args} -create -output ../robotraconteur_and_deps-ios.a", shell=True, cwd=combined_lib_dir_tmp)
sim_lib_args = " ".join([f"robotraconteur_and_deps-{c1}.a" for c1 in combined_lipo_sim])
subprocess.check_call(f"lipo {sim_lib_args} -create -output ../robotraconteur_and_deps-iphonesimulator.a", shell=True, cwd=combined_lib_dir_tmp)
...
if 'x64-ios' in triplet:
    plist= plist_in.replace("<!--<string>x86_64</string>-->","<string>x86_64</string>")
else:
    plist= plist_in.replace("<!--<string>arm64</string>-->","<string>arm64</string>")
