...
public class NamedMultiDimArrayMemoryClient<T> extends NamedMultiDimArrayMemory<T>
{
    ...
    class bufferdirector extends WrappedNamedMultiDimArrayMemoryClientBuffer
    {
        ...
        @Override
        public void unpackReadResult(MessageElementNestedElementList res, vector_uint64_t bufferpos,
                                     vector_uint64_t count)
        {
            try
            {
                NamedMultiDimArray res1 = (NamedMultiDimArray)RobotRaconteurNode.s().unpackStructure(res);
                buffer.assignSubArray(vector_uint64_to_int(bufferpos), res1, new int[buffer.dims.length],
                                      vector_uint64_to_int(count));
            }
            finally
            {
                res.finalize();
    ...
    @Override public void read(long[] memorypos, NamedMultiDimArray buffer, long[] bufferpos, long[] count)
    {
        bufferdirector buffer1 = new bufferdirector(buffer);
        try
        {
            innerclient.read(long_to_vector_uint64(memorypos), buffer1, long_to_vector_uint64(bufferpos),
                             long_to_vector_uint64(count));
        }
        finally
        {
            buffer1.finalize();
    ...
    @Override public void write(long[] memorypos, NamedMultiDimArray buffer, long[] bufferpos, long[] count)
    {
        bufferdirector buffer1 = new bufferdirector(buffer);
        try
        {
            innerclient.write(long_to_vector_uint64(memorypos), buffer1, long_to_vector_uint64(bufferpos),
                              long_to_vector_uint64(count));
        }
        finally
        {
