...
//ORIGINAL LINE: public override void Read(ulong memorypos, T[] buffer, ulong bufferpos, ulong count)
	...
	public void read(long memorypos, T buffer, long bufferpos, long count)
	{
		...
		try
		{
		dat= WrappedArrayMemoryClientUtil.read(innerarray, BigInteger.valueOf(memorypos), BigInteger.valueOf(count));
		T dat2 = (T)MessageElementDataUtil.rRBaseArrayToArray(dat);
		DataTypeUtil.arraycopy(dat2, 0, buffer, (int)bufferpos, (int)count);
		}
		finally
		{
			if (dat!=null) dat.delete();
...
//ORIGINAL LINE: if ((ulong)buffer.Length == count)
		...
		WrappedArrayMemoryClientUtil.write(innerarray, BigInteger.valueOf(memorypos), buffer2, BigInteger.valueOf(0), BigInteger.valueOf(count));
		}
		finally
		{
