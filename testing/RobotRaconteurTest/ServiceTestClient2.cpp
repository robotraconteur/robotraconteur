...
void ServiceTestClient2::Disconnect() { RobotRaconteurNode::s()->DisconnectService(r); }
...
{
    r->get_peekwire()->AsyncPeekInValue(boost::bind(&ServiceTestClient2::AsyncTestWirePeekPoke1, this,
                                                ...
                                                RR_SHARED_PTR<RobotRaconteurException> err)
{
    ...
    r->get_pokewire()->AsyncPokeOutValue(
...
void ServiceTestClient2::AsyncTestWirePeekPoke2(RR_SHARED_PTR<RobotRaconteurException> err)
{
    ...
    r->get_pokewire()->AsyncPeekOutValue(boost::bind(&ServiceTestClient2::AsyncTestWirePeekPoke3, this,
                                                     RR_BOOST_PLACEHOLDERS(_1), RR_BOOST_PLACEHOLDERS(_2),
                                                     RR_BOOST_PLACEHOLDERS(_3)));
}
...
void ServiceTestClient2::TestGenerators()
{
    ...
    RR_INTRUSIVE_PTR<RRArray<uint8_t> > b = gen->Next(AttachRRArrayCopy(a, 3));
    gen->Abort();
    try
    {
        gen->Next(AttachRRArrayCopy(a, 3));
...
void ServiceTestClient2::TestNamedArrays()
{
    ...
    com::robotraconteur::testing::TestService3::transform a2_1 = r->get_testnamedarray2();
...
void ServiceTestClient2::TestComplex()
{
    ...
    RR_INTRUSIVE_PTR<RRArray<cdouble> > c2_1 = r->get_c2();
    ...
    RR_INTRUSIVE_PTR<RRMultiDimArray<cdouble> > c3_1 = r->get_c3();
    uint32_t c3_1_1[] = {2, 5};
    ...
    r->set_c3(AllocateRRMultiDimArray<cdouble>(AttachRRArrayCopy(c3_2_1, 2), AttachRRArrayCopy((cdouble*)c3_2_2, 12)));

    RR_INTRUSIVE_PTR<RRList<RRArray<cdouble> > > c5_1 = r->get_c5();
    if (!c5_1)
        throw std::runtime_error("");
    ...
    RR_INTRUSIVE_PTR<RRArray<cfloat> > c8_1 = r->get_c8();
    ...
    r->set_c9(AllocateRRMultiDimArray<cfloat>(AttachRRArrayCopy(c9_2_1, 3), AttachRRArrayCopy((cfloat*)c9_2_2, 8)));
}

void ServiceTestClient2::TestComplexMemories()
{
    double c_m1_1[] = {8.952764e-05,  4.348213e-04,  -1.051215e+08, 1.458626e-09, -2.575954e+10, 2.118740e+03,
    ...
    uint32_t c_m2_1[] = {3, 3};
    double c_m2_2[] = {-4.850043e-03, 3.545429e-07,  2.169430e+12,  1.175943e-09,  2.622300e+08, -4.439823e-11,
    ...
    RR_INTRUSIVE_PTR<RRMultiDimArray<cdouble> > c_m2_3 =
        AllocateRRMultiDimArray<cdouble>(AttachRRArrayCopy(c_m2_1, 2), AttachRRArrayCopy((cdouble*)c_m2_2, 9));
    r->get_c_m2()->Write(z, c_m2_3, z, c);

    RR_INTRUSIVE_PTR<RRMultiDimArray<cdouble> > c_m2_4 =
        AllocateRRMultiDimArray<cdouble>(AttachRRArrayCopy(c_m2_1, 2), AllocateRRArray<cdouble>(9));
...
#define ShouldBeLockedErr(COMMAND)                                                                                     \
    {                                                                                                                  \
        bool err = false;                                                                                              \
        try                                                                                                            \
        {                                                                                                              \
...
void ServiceTestClient2::TestBools()
{
    ...
    if (v4_1->size() != 1)
        throw std::runtime_error("");
    if (RRArrayToScalar(v4_1->front()) != 1)
        throw std::runtime_error("");
...
void ServiceTestClient2::TestBoolMemories()
{
    ...
    RR_INTRUSIVE_PTR<RRMultiDimArray<rr_bool> > v3 =
        AllocateRRMultiDimArray(AttachRRArrayCopy(v3_dims, 2), AttachRRArrayCopy(v1_1, 10));
...
void ServiceTestClient2::TestExceptionParams()
{
    ...
    catch (InvalidOperationException& exp)
    {
        ...
        RR_INTRUSIVE_PTR<RRMap<std::string, RRValue> > param_map =
            rr_cast<RRMap<std::string, RRValue> >(exp.ErrorParam);
        if (param_map->size() != 2)
        {
            throw std::runtime_error("");
        }
