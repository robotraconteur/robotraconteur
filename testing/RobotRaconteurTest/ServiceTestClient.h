...
class ServiceTestClient
{
  public:
    ...
    void TestAsync5(const RR_SHARED_PTR<async_testroot>& r, double ret,
                    const RR_SHARED_PTR<RobotRaconteurException>& exp);
    ...
    void TestAsync9(const RR_SHARED_PTR<testroot>& r,
                    RR_SHARED_PTR<PipeEndpoint<RR_INTRUSIVE_PTR<RRArray<double> > > > e1, uint32_t pnum,
                    const RR_SHARED_PTR<RobotRaconteurException>& exp);

    void TestAsync10(const RR_SHARED_PTR<testroot>& r,
                     RR_SHARED_PTR<PipeEndpoint<RR_INTRUSIVE_PTR<RRArray<double> > > > e1,
                     const RR_SHARED_PTR<RobotRaconteurException>& exp);

    void TestAsync11(const RR_SHARED_PTR<testroot>& r,
                     RR_SHARED_PTR<WireConnection<RR_INTRUSIVE_PTR<RRArray<double> > > > w1,
                     const RR_SHARED_PTR<RobotRaconteurException>& exp);

    void TestAsync12(const RR_SHARED_PTR<testroot>& r,
                     RR_SHARED_PTR<WireConnection<RR_INTRUSIVE_PTR<RRArray<double> > > > w1,
                     const RR_SHARED_PTR<RobotRaconteurException>& exp);
