...
class Reynard_impl:
    ...
    def setf_arm_position(self, q1, q2, q3):
        res = requests.post(self._base_url + "/api/arm", json={"q1": np.rad2deg(q1), "q2": np.rad2deg(q2), "q3": np.rad2deg(q3)})
        res.raise_for_status()
    ...
    def drive_robot(self, vel_x, vel_y, timeout=-1, wait=False):
        res = requests.post(self._base_url + "/api/drive_robot", json={"vel_x": vel_x*1000.0, "vel_y": vel_y*1000.0, "timeout": timeout, "wait": wait})
        res.raise_for_status()

    def drive_arm(self, q1, q2, q3, timeout=-1, wait=False):
        res = requests.post(self._base_url + "/api/drive_arm", json={"q1": np.rad2deg(q1), "q2": np.rad2deg(q2), "q3": np.rad2deg(q3), "timeout": timeout, "wait": wait})
        res.raise_for_status()
    ...
    def _timer_cb(self, ev):
        try:
            ...
            state.arm_position = np.deg2rad(np.array([res_json["q1"], res_json["q2"], res_json["q3"]], dtype=np.float64))
            state.robot_velocity = 1e-3*np.array([res_json["vel_x"], res_json["vel_y"]], dtype=np.float64)
            state.arm_velocity = np.deg2rad(np.array([res_json["vel_q1"], res_json["vel_q2"], res_json["vel_q3"]], dtype=np.float64))
