...
class Reynard_impl : Reynard_default_impl, IDisposable
{
    ...
    public override double[] getf_arm_position()
    {
        ...
        return new double[]
        {
            json.q1 * (Math.PI/180.0),
            json.q2 * (Math.PI/180.0),
    ...
    private void _timer_cb(TimerEvent evt)
    {
        try
        {
            ...
            var state = new ReynardState()
            {
                ...
                arm_position = new double[] { json.q1 * (Math.PI / 180.0), json.q2 * (Math.PI / 180.0), json.q3 * (Math.PI / 180.0) },
                robot_velocity = new double[] { json.vel_x * 1e-3, json.vel_y * 1e-3 },
                arm_velocity = new double[] { json.vel_q1 * (Math.PI / 180.0), json.vel_q2 * (Math.PI / 180.0), json.vel_q3 * (Math.PI / 180.0) }
            };
...
class Program
{
    static int Main(string[] args)
    {
        ...
        using (reynard)
        using (var node_setup = new ServerNodeSetup("experimental.reynard_the_robot_csharp_rest", 59201, args))
        {
