...
TEST(RobotRaconteurMisc, EncodeServicePathIndexTest)
{
    ...
    EXPECT_EQ(EncodeServicePathIndex("gamma_with_special!%$moreWords()*.[]"), "gamma%5fwith%5fspecial%21%25%24moreWords%28%29%2a%2e%5b%5d");
    EXPECT_EQ(DecodeServicePathIndex("delta%5fwith%5fspecial%3F%0A%5C%2d%21%25%24More%2E%5B%5Dwords%28%29%2A"), "delta_with_special?\n\\-!%$More.[]words()*");
...
TEST(RobotRaconteurMisc, BuildServicePathTest)
{
    ...
    segments.push_back(ServicePathSegment("delta",std::string("delta_index_with_special!%$morewords()*.[]\n\t4927`\"?-")));
    segments.push_back(ServicePathSegment("epsilon",std::string("38563")));

    std::string path = BuildServicePath(segments);
    EXPECT_EQ(path, "alpha.be2_ta.gamma_31[gamma%5findex].delta[delta%5findex%5fwith%5fspecial%21%25%24morewords%28%29%2a%2e%5b%5d%0a%094927%60%22%3f%2d].epsilon[38563]");

    std::vector<ServicePathSegment> segments2 = ParseServicePath(path);
    EXPECT_EQ(segments, segments2);

    std::string path2_str = "zeta[alpha].eta1[%2D0175].Theta_3[gamma%5findex].iota[%5B%5D%21bunch%2Dof%2Dgarbage%5f%0928576%0A%21%40%23%24%25%5E%26%2A%28%29%2e%0D%5c%2f].kappa";
