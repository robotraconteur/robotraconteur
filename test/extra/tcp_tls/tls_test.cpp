...
{
    public:
        RR_OVIRTUAL double add_two_numbers(double a, double b) RR_OVERRIDE
        {
            return a+b;
        }
};
...
TEST(RobotRaconteurTLS, RobotRaconteurTLS)
{
    ...
    RobotRaconteurNodeSetupFlags tls_client_flags = (RobotRaconteurNodeSetupFlags) \
    ...
    RR_SHARED_PTR<RobotRaconteurNodeSetup> node1_setup = RR_MAKE_SHARED<SecureServerNodeSetup>(node1, 
        ROBOTRACONTEUR_SERVICE_TYPES, "testprog_tls1");
    RR_SHARED_PTR<RobotRaconteurNodeSetup> node2_setup = RR_MAKE_SHARED<ClientNodeSetup>(node2, ROBOTRACONTEUR_SERVICE_TYPES);
    RR_SHARED_PTR<RobotRaconteurNodeSetup> node3_setup = RR_MAKE_SHARED<RobotRaconteurNodeSetup>(node3, 
        ROBOTRACONTEUR_SERVICE_TYPES, "testprog_tls2", 0, tls_client_flags);
    RR_SHARED_PTR<RobotRaconteurNodeSetup> node4_setup = RR_MAKE_SHARED<RobotRaconteurNodeSetup>(node4, 
        ROBOTRACONTEUR_SERVICE_TYPES, "testprog_tls3", 0, tls_client_flags);
    ...
    std::string authdata = "testuser1 0b91dec4fe98266a03b136b59219d0d6 objectlock " + node3->NodeID().ToString() + "\n"
        + "testuser2 841c4221c2e7e0cefbc0392a35222512 objectlock " + node4->NodeID().ToString() + "\n" 
        + "testsuperuser 503ed776c50169f681ad7bbc14198b68 objectlock,objectlockoverride " + node3->NodeID().ToString() 
        + "\n";
    ...
    ASSERT_NO_THROW(
        c1 = rr_cast<experimental::tls_test::testobj>(node2->ConnectService(url));
        EXPECT_EQ(c1->add_two_numbers(5,6), 11);
        node1->DisconnectService(c1);
    ); 

    ASSERT_THROW(
        rr_cast<experimental::tls_test::testobj>(node2->ConnectService(url_unsecure));
    , ConnectionException);

    ASSERT_NO_THROW(
        c1 = rr_cast<experimental::tls_test::testobj>(node2->ConnectService(url_ws));
        EXPECT_EQ(c1->add_two_numbers(5,6), 11);
        node1->DisconnectService(c1);
    ); 

    ASSERT_THROW(
        rr_cast<experimental::tls_test::testobj>(node2->ConnectService(url_unsecure_ws));
    , ConnectionException);
    ...
    ASSERT_NO_THROW(
        c1 = rr_cast<experimental::tls_test::testobj>(node3->ConnectService(url_secure, user1, cred1));
        EXPECT_EQ(c1->add_two_numbers(5,7), 12);
        node1->DisconnectService(c1);
    ); 
    ...
    ASSERT_NO_THROW(
        c1 = rr_cast<experimental::tls_test::testobj>(node4->ConnectService(url_secure, user2, cred2));
        EXPECT_EQ(c1->add_two_numbers(9,7), 16);
        node1->DisconnectService(c1);
    ); 
